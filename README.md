...
## v3.2.13.3 — Recuperação definitiva

O Service Worker foi refeito para nunca devolver valor vazio ao navegador.
Também foram incluídos cache individual dos arquivos, retorno válido para
falhas de rede, proteção de inicialização e a página `reparar-cache.html`.

As regras do Firestore permanecem inalteradas.


## v3.2.13.4 — Modo offline restaurado

O arquivo `js/offline.js` passa a fazer parte obrigatória do patch e do cache
crítico do Service Worker.

A nova versão:

- não ativa o Service Worker sem armazenar `offline.js`;
- tenta novamente o arquivo sem o parâmetro de versão;
- preserva caches anteriores se o cache crítico estiver incompleto;
- nunca devolve erro 503 para arquivos JavaScript ou CSS;
- mantém um fallback online apenas como última proteção.
