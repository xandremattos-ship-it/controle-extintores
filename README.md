...
## Etapa 4 — Conflitos e segurança

- versionamento otimista por documento;
- transações Firestore antes de atualizar registros;
- identificação de usuário, perfil, usina e dispositivo;
- central de conflitos com comparação campo a campo;
- opções para manter a nuvem ou substituir pela versão local;
- exclusões protegidas por versão;
- trilha de auditoria em `auditLogs`;
- operações locais não apagam documentos da nuvem por ausência em snapshot;
- conflito não é tratado como falha comum e exige decisão do usuário.

### Regras adicionais

Inclua o conteúdo de `firestore-audit-rules.txt` nas regras do Firestore.
