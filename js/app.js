...
function openHydrantModal(hydrantId=null){
  const existing=hydrantId ? STATE.hydrants.find(h=>String(h.id)===String(hydrantId)) : null;
  const usinas=hasAllUsinasAccess()?STATE.config.usinas:[currentUserUsina()];
  const data=existing || {numero:'',localizacao:'',usina:usinas[0]||'',ultimoTesteMangueira:'',statusGeral:'EM CAMPO',foto:null};

  openModal(`<div class="modal">
    <div class="modal-head"><h3>${existing?'Editar hidrante':'Novo hidrante'}</h3><button class="icon-btn" id="hydrant-modal-close">${ICONS.close}</button></div>
    <div class="modal-body">
      <div class="form-grid">
        <div class="field"><label>Número do hidrante</label><input id="hydrant-number" value="${escapeHtml(data.numero)}" inputmode="numeric"></div>
        <div class="field"><label>Usina</label><select id="hydrant-plant" ${hasAllUsinasAccess()?'':'disabled'}>${usinas.map(u=>`<option value="${escapeHtml(u)}" ${data.usina===u?'selected':''}>${escapeHtml(u)}</option>`).join('')}</select></div>
        <div class="field span-2"><label>Localização</label><input id="hydrant-location" value="${escapeHtml(data.localizacao)}"></div>
        <div class="field"><label>Último teste da mangueira</label><input type="month" id="hydrant-hose-test" value="${escapeHtml(data.ultimoTesteMangueira||'')}"></div>
        <div class="field"><label>Situação</label><select id="hydrant-operational"><option value="EM CAMPO" ${data.statusGeral!=='DESATIVADO'?'selected':''}>Em campo</option><option value="DESATIVADO" ${data.statusGeral==='DESATIVADO'?'selected':''}>Desativado</option></select></div>
        <div class="field span-2"><label>Foto do hidrante/abrigo</label><input type="file" accept="image/*" id="hydrant-photo"><small>A foto será compactada antes de salvar.</small></div>
      </div>
    </div>
    <div class="modal-foot"><button class="btn" id="hydrant-cancel">Cancelar</button><button class="btn btn-accent" id="hydrant-save">Salvar hidrante</button></div>
  </div>`);

  document.getElementById('hydrant-modal-close').onclick=closeModal;
  document.getElementById('hydrant-cancel').onclick=closeModal;
  document.getElementById('hydrant-save').onclick=async()=>{
    const numero=document.getElementById('hydrant-number').value.trim();
    const localizacao=document.getElementById('hydrant-location').value.trim();
    const usina=hasAllUsinasAccess()?document.getElementById('hydrant-plant').value:currentUserUsina();
    if(!numero || !localizacao || !usina){showToast('Preencha número, localização e usina.','err');return;}
    const duplicate=STATE.hydrants.find(h=>String(h.numero).trim()===numero && h.usina===usina && String(h.id)!==String(existing?.id||''));
    if(duplicate){showToast('Já existe um hidrante com este número nessa usina.','err');return;}
    let foto=data.foto||null;
    const file=document.getElementById('hydrant-photo').files?.[0];
    if(file) foto=await fileToDataUrl(file);
    const record=markPendingSync({
      ...data,
      id:existing?.id || uid('HID'),
      numero,localizacao,usina,
      ultimoTesteMangueira:document.getElementById('hydrant-hose-test').value,
      statusGeral:document.getElementById('hydrant-operational').value,
      foto,
      createdAt:existing?.createdAt || new Date().toISOString(),
      updatedAt:new Date().toISOString()
    });
    const index=STATE.hydrants.findIndex(h=>String(h.id)===String(record.id));
    if(index>=0) STATE.hydrants[index]=record; else STATE.hydrants.push(record);
    await persistHydrants(); closeModal(); showToast(existing?'Hidrante atualizado.':'Hidrante cadastrado.'); renderView();
  };
}
...
function openHydrantInspectionModal(hydrantId,inspectionId=null){
  const hydrant=STATE.hydrants.find(h=>String(h.id)===String(hydrantId));
  if(!hydrant) return;
  const existing=inspectionId?STATE.hydrantInspections.find(i=>String(i.id)===String(inspectionId)):null;
  const answers={};
  HYDRANT_CHECK_ITEMS.forEach(item=>answers[item]=existing?.checklist?.find(row=>row.item===item)?.valor || 'C');

  openModal(`<div class="modal modal-wide">
    <div class="modal-head"><div><h3>Inspeção do hidrante #${escapeHtml(hydrant.numero)}</h3><small>${escapeHtml(hydrant.localizacao)} · ${escapeHtml(hydrant.usina)}</small></div><button class="icon-btn" id="hydrant-inspection-close">${ICONS.close}</button></div>
    <div class="modal-body">
      <div class="form-grid">
        <div class="field"><label>Data da inspeção</label><input type="date" id="hydrant-inspection-date" value="${existing?.date || new Date().toISOString().slice(0,10)}"></div>
        <div class="field"><label>Responsável</label><input id="hydrant-inspector" value="${escapeHtml(existing?.responsavel || CURRENT_USER_PROFILE?.name || '')}"></div>
      </div>
      <div class="hydrant-checklist">
        ${HYDRANT_CHECK_ITEMS.map(item=>{
          const row=existing?.checklist?.find(entry=>entry.item===item);
          return `<div class="hydrant-check-row" data-hydrant-check="${escapeHtml(item)}">
            <div><b>${escapeHtml(item)}</b>${HYDRANT_QUANTITY_ITEMS.has(item)?`<label class="hydrant-qty">Quantidade <input type="number" min="0" data-hydrant-qty="${escapeHtml(item)}" value="${row?.quantidade ?? ''}"></label>`:''}</div>
            <div class="hydrant-check-options">
              ${['C','NC','NA'].map(value=>`<button type="button" class="check-option ${answers[item]===value?'active':''}" data-item="${escapeHtml(item)}" data-value="${value}">${value==='NA'?'N.A.':value}</button>`).join('')}
            </div>
          </div>`;
        }).join('')}
      </div>
      <div class="field"><label>Observação</label><textarea id="hydrant-observation" rows="3">${escapeHtml(existing?.observacao||'')}</textarea></div>
      <section class="hydrant-treatment ${Object.values(answers).includes('NC')?'show':''}" id="hydrant-treatment-section">
        <h4>Tratativa da não conformidade</h4>
        <div class="form-grid">
          <div class="field"><label>Setor responsável</label><select id="hydrant-nc-sector"><option value="">Selecione</option>${NC_SETORES.map(s=>`<option ${existing?.naoConformidade?.setor===s?'selected':''}>${s}</option>`).join('')}</select></div>
          <div class="field"><label>Responsável</label><input id="hydrant-nc-person" value="${escapeHtml(existing?.naoConformidade?.responsavel||'')}"></div>
          <div class="field"><label>Cargo</label><input id="hydrant-nc-role" value="${escapeHtml(existing?.naoConformidade?.cargo||'')}"></div>
          <div class="field"><label>Data do encaminhamento</label><input type="date" id="hydrant-nc-date" value="${existing?.naoConformidade?.dataEncaminhamento || new Date().toISOString().slice(0,10)}"></div>
          <div class="field"><label>Forma de comunicação</label><select id="hydrant-nc-channel"><option value="">Selecione</option>${NC_FORMAS_COMUNICACAO.map(s=>`<option ${existing?.naoConformidade?.formaComunicacao===s?'selected':''}>${s}</option>`).join('')}</select></div>
          <div class="field"><label>Número da OS</label><input id="hydrant-nc-os" value="${escapeHtml(existing?.naoConformidade?.numeroOS||'')}"></div>
          <div class="field span-2"><label>Descrição do encaminhamento</label><textarea id="hydrant-nc-description" rows="3">${escapeHtml(existing?.naoConformidade?.descricao||'')}</textarea></div>
        </div>
      </section>
    </div>
    <div class="modal-foot"><button class="btn" id="hydrant-inspection-cancel">Cancelar</button><button class="btn btn-accent" id="hydrant-inspection-save">Salvar inspeção</button></div>
  </div>`);

  const syncTreatment=()=>document.getElementById('hydrant-treatment-section').classList.toggle('show',Object.values(answers).includes('NC'));
  document.querySelectorAll('.check-option').forEach(button=>button.addEventListener('click',()=>{
    answers[button.dataset.item]=button.dataset.value;
    button.closest('.hydrant-check-options').querySelectorAll('.check-option').forEach(b=>b.classList.toggle('active',b===button));
    syncTreatment();
  }));
  document.getElementById('hydrant-inspection-close').onclick=closeModal;
  document.getElementById('hydrant-inspection-cancel').onclick=closeModal;
  document.getElementById('hydrant-inspection-save').onclick=async()=>{
    const date=document.getElementById('hydrant-inspection-date').value;
    const responsavel=document.getElementById('hydrant-inspector').value.trim();
    if(!date || !responsavel){showToast('Preencha a data e o responsável.','err');return;}
    const hasNc=Object.values(answers).includes('NC');
    let nc=null;
    if(hasNc){
      const setor=document.getElementById('hydrant-nc-sector').value;
      const pessoa=document.getElementById('hydrant-nc-person').value.trim();
      const descricao=document.getElementById('hydrant-nc-description').value.trim();
      if(!setor || !pessoa || !descricao){showToast('Preencha setor, responsável e descrição da tratativa.','err');return;}
      nc={
        ...(existing?.naoConformidade||{}),
        status:existing?.naoConformidade?.status || 'ENCAMINHADA',
        setor,responsavel:pessoa,
        cargo:document.getElementById('hydrant-nc-role').value.trim(),
        dataEncaminhamento:document.getElementById('hydrant-nc-date').value,
        formaComunicacao:document.getElementById('hydrant-nc-channel').value,
        numeroOS:document.getElementById('hydrant-nc-os').value.trim(),
        descricao,
        criadoEm:existing?.naoConformidade?.criadoEm || new Date().toISOString(),
        atualizadoEm:new Date().toISOString()
      };
    }
    const checklist=HYDRANT_CHECK_ITEMS.map(item=>({
      item,valor:answers[item],
      ...(HYDRANT_QUANTITY_ITEMS.has(item)?{quantidade:Number(document.querySelector(`[data-hydrant-qty="${item}"]`)?.value||0)}:{})
    }));
    const record=markPendingSync({
      ...(existing||{}),id:existing?.id||uid('HINSP'),hydrantId:hydrant.id,usina:hydrant.usina,date,responsavel,
      checklist,observacao:document.getElementById('hydrant-observation').value.trim(),
      resultado:hasNc?'NÃO CONFORME':'CONFORME',naoConformidade:nc,
      createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()
    });
    const index=STATE.hydrantInspections.findIndex(i=>String(i.id)===String(record.id));
    if(index>=0) STATE.hydrantInspections[index]=record; else STATE.hydrantInspections.push(record);
    await persistHydrantInspections(); closeModal(); showToast(existing?'Inspeção atualizada.':'Inspeção registrada.'); renderView();
  };
}

function renderHydrantDetailPage(){
  const hydrant=STATE.hydrants.find(h=>String(h.id)===String(STATE.selectedHydrantId));
  if(!hydrant){STATE.view='hydrants';return renderShell();}
  const history=hydrantInspectionsFor(hydrant.id);
  const latest=history[0];
  const result=hydrantResultMeta(latest);
  setHeader(`Hidrante #${hydrant.numero}`,`${hydrant.localizacao} · ${hydrant.usina}`,`<button class="btn" id="hydrant-back">← Voltar</button>${can('createHydrantInspections')?`<button class="btn btn-accent" id="hydrant-detail-inspect">${ICONS.history} Nova inspeção</button>`:''}`);
  const content=document.getElementById('content');
  content.innerHTML=`
    <div class="hydrant-detail-head">
      <div class="hydrant-photo large">${hydrant.foto?`<img src="${hydrant.foto}" alt="">`:ICONS.hydrant}</div>
      <div><span class="eyebrow">Prontuário digital</span><h2>Hidrante #${escapeHtml(hydrant.numero)}</h2><p>${escapeHtml(hydrant.localizacao)} · ${escapeHtml(hydrant.usina)}</p></div>
      <div class="hydrant-detail-actions">${canEditHydrant(hydrant)?`<button class="btn" id="hydrant-detail-edit">${ICONS.edit} Editar</button>`:''}<button class="btn" id="hydrant-detail-report">${ICONS.report} PDF individual</button></div>
    </div>
    <div class="hydrant-detail-grid">
      <div class="panel"><span>Último teste da mangueira</span><b>${formatMonthYear(hydrant.ultimoTesteMangueira)}</b></div>
      <div class="panel"><span>Situação</span><b class="${hydrantStatusMeta(hydrant.statusGeral).cls}">${hydrantStatusMeta(hydrant.statusGeral).label}</b></div>
      <div class="panel"><span>Última inspeção</span><b>${latest?fmtDate(latest.date):'—'}</b></div>
      <div class="panel"><span>Resultado</span><b class="${result.cls}">${result.label}</b></div>
    </div>
    <section class="panel">
      <div class="section-head"><h3>Histórico de inspeções</h3><span>${history.length} registro(s)</span></div>
      ${history.length?`<div class="hydrant-history">${history.map(ins=>{
        const meta=hydrantResultMeta(ins); const nc=ins.naoConformidade;
        return `<article class="hydrant-history-item">
          <div><b>${fmtDate(ins.date)}</b><span>${escapeHtml(ins.responsavel)}</span></div>
          <div><small class="${meta.cls}">${meta.label}</small>${nc?`<span>Tratativa: ${escapeHtml(nc.status||'ENCAMINHADA')}</span>`:''}</div>
          <div class="hydrant-history-actions"><button class="btn btn-sm" data-hydrant-inspection-view="${ins.id}">Ver checklist</button>${can('editHydrantInspections')?`<button class="icon-btn" data-hydrant-inspection-edit="${ins.id}">${ICONS.edit}</button>`:''}${nc && nc.status!=='RESOLVIDA' && can('editHydrantInspections')?`<button class="btn btn-sm" data-hydrant-treatment="${ins.id}">Atualizar tratativa</button>`:''}</div>
        </article>`;
      }).join('')}</div>`:'<div class="empty"><h4>Sem inspeções</h4><p>Registre a primeira inspeção deste hidrante.</p></div>'}
    </section>`;
  document.getElementById('hydrant-back').onclick=()=>{STATE.view='hydrants';renderShell();};
  document.getElementById('hydrant-detail-inspect')?.addEventListener('click',()=>openHydrantInspectionModal(hydrant.id));
  document.getElementById('hydrant-detail-edit')?.addEventListener('click',()=>openHydrantModal(hydrant.id));
  document.getElementById('hydrant-detail-report')?.addEventListener('click',()=>generateHydrantIndividualReport(hydrant));
  content.querySelectorAll('[data-hydrant-inspection-view]').forEach(b=>b.addEventListener('click',()=>openHydrantInspectionView(b.dataset.hydrantInspectionView)));
  content.querySelectorAll('[data-hydrant-inspection-edit]').forEach(b=>b.addEventListener('click',()=>openHydrantInspectionModal(hydrant.id,b.dataset.hydrantInspectionEdit)));
  content.querySelectorAll('[data-hydrant-treatment]').forEach(b=>b.addEventListener('click',()=>openHydrantTreatmentModal(b.dataset.hydrantTreatment)));
}
...
function openHydrantTreatmentModal(inspectionId){
  ...
  const nc=inspection.naoConformidade;
  openModal(`<div class="modal"><div class="modal-head"><h3>Atualizar tratativa</h3><button class="icon-btn" id="hydrant-treatment-close">${ICONS.close}</button></div><div class="modal-body"><div class="field"><label>Status</label><select id="hydrant-treatment-status"><option value="ENCAMINHADA" ${nc.status==='ENCAMINHADA'?'selected':''}>Tratativa encaminhada</option><option value="EM ANDAMENTO" ${nc.status==='EM ANDAMENTO'?'selected':''}>Em andamento</option><option value="RESOLVIDA" ${nc.status==='RESOLVIDA'?'selected':''}>Resolvida</option></select></div><div class="field"><label>Atualização/observação</label><textarea id="hydrant-treatment-note" rows="4">${escapeHtml(nc.atualizacao||'')}</textarea></div></div><div class="modal-foot"><button class="btn" id="hydrant-treatment-cancel">Cancelar</button><button class="btn btn-accent" id="hydrant-treatment-save">Salvar</button></div></div>`);
...
function hydrantReportRows(list){
  return list.map(h=>{
    const latest=latestHydrantInspection(h.id);
    const quantities={};
    latest?.checklist?.forEach(row=>{if(HYDRANT_QUANTITY_ITEMS.has(row.item)) quantities[row.item]=row.quantidade??0;});
    return [h.numero,h.usina,h.localizacao,formatMonthYear(h.ultimoTesteMangueira),latest?fmtDate(latest.date):'—',latest?.resultado||'SEM INSPEÇÃO',quantities['Esguichos']??'—',quantities['Chave Storz']??'—',quantities['Mangueiras']??'—',h.statusGeral||'EM CAMPO'];
  });
}
...
function generateHydrantReport(){
  const {jsPDF}=window.jspdf;
  const doc=new jsPDF({orientation:'landscape',unit:'pt',format:'a4'});
  doc.setFont('helvetica','bold');doc.setFontSize(16);doc.text('RELATÓRIO GERAL DE HIDRANTES',40,38);
  doc.setFont('helvetica','normal');doc.setFontSize(9);doc.text(`Gerado em ${new Date().toLocaleString('pt-BR')} · ${accessScopeLabel()}`,40,56);
  let list=[...STATE.hydrants];
  if(hydrantFilters.usina) list=list.filter(h=>h.usina===hydrantFilters.usina);
  doc.autoTable({startY:72,head:[['Nº','Usina','Localização','Teste mangueira','Última inspeção','Resultado','Esg.','Storz','Mang.','Situação']],body:hydrantReportRows(list),styles:{fontSize:7,cellPadding:4},headStyles:{fillColor:[56,122,70]},didParseCell(data){if(data.section!=='body')return;const row=data.row.raw;if(row[5]==='NÃO CONFORME'||row[9]==='DESATIVADO')data.cell.styles.fillColor=[255,239,237];else if(row[5]==='CONFORME')data.cell.styles.fillColor=[235,248,239];}});
  doc.save(`relatorio-hidrantes-${new Date().toISOString().slice(0,10)}.pdf`);
}
...
function generateHydrantIndividualReport(hydrant){
  ...
  const body=[];history.forEach(ins=>ins.checklist.forEach(row=>body.push([fmtDate(ins.date),ins.responsavel,row.item,row.valor==='NA'?'N.A.':row.valor,HYDRANT_QUANTITY_ITEMS.has(row.item)?String(row.quantidade??0):'—'])));
  doc.autoTable({startY:96,head:[['Data','Responsável','Item','Resultado','Qtd.']],body,styles:{fontSize:8},headStyles:{fillColor:[56,122,70]}});
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v4.0.0 · Módulo Hidrantes</div>
          </div>
        </div>
