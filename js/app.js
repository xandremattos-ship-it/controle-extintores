...
function sortNotificationsNewestFirst(items){
  return [...items].sort((a,b)=>{
    const dateDifference =
      notificationTimestamp(b.createdAt) -
      notificationTimestamp(a.createdAt);
...
async function buildNotificationItems(){
  ...
  STATE.extinguishers.forEach(extinguisher=>{
    ...
    if(rechargeDays!==null && rechargeDays<0){
      addNotificationItem(items,{
        ...
        createdAt:`${extinguisher.dataRecarga || todayISO()}T12:00:00`,
        action:{
          type:'extinguisher',
    ...
    if(hydroDays!==null && hydroDays<0){
      addNotificationItem(items,{
        ...
        createdAt:`${extinguisher.dataTH || todayISO()}T12:00:00`,
        action:{
          type:'extinguisher',
          extinguisherId:String(extinguisher.id)
        },
      ...
      if(status==='ENCAMINHADA'){
        addNotificationItem(items,{
          ...
          createdAt:nc.atualizadoEm ||
            nc.criadoEm ||
            `${inspection.date || todayISO()}T12:00:00`,
          action:{
            type:'inspection',
  ...
  const readIds = loadNotificationReadIds();
  const currentIds = new Set(chronologicalItems.map(item=>String(item.id)));
    ...
    <article
      ...
      <div class="notification-item-content">
        ...
        <div class="notification-item-meta">
          ...
          ${item.createdAt
            ? `<time>${escapeHtml(notificationDateLabel(item.createdAt))}</time>`
            : ''
          }
        </div>
...
async function renderNotificationCenterContent(){
  ...
  content.innerHTML = `
    ...
    <div class="notification-toolbar">
      <span>
        ${filtered.length} resultado(s) · mais recentes primeiro
      </span>
      <div>
        <button
          type="button"
...
function bindNotificationCenterContent(){
  ...
  document.getElementById('notification-mark-all')
    ?.addEventListener('click',()=>{
      markAllNotificationsRead(NOTIFICATION_CACHE);
      renderNotificationCenterContent();
    });
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v3.2.11.3 · Notificações Cronológicas</div>
          </div>
        </div>
