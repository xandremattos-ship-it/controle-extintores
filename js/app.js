  ...
  await enqueueOfflineOperation({
    ...
    storagePaths:
      hydrant?.fotoStoragePath
        ? [String(hydrant.fotoStoragePath)]
        : []
  });
}
...
function cardExt(e){
  ...
  <div class="ext-card status-${st.cls}" data-view-ext="${e.id}" style="cursor:pointer;">
    <div class="ext-photo">
      ${e.foto?`<img src="${e.foto}" loading="lazy" decoding="async">`:`<div class="no-photo">${ICONS.photo}</div>`}
  ...
  return `
    <article
      ...
      <div class="ext-list-photo">
        ${e.foto
          ? `<img src="${e.foto}" alt="Foto do extintor ${escapeHtml(String(e.numero))}" loading="lazy" decoding="async">`
...
async function deleteExtinguisher(id){
  ...
  await enqueueOfflineOperation({
    ...
    storagePaths:ext.fotoStoragePath
      ? [String(ext.fotoStoragePath)]
      : []
  });
  ...
  content.innerHTML = `
    ...
    ${list.length ? `<div class="hydrant-grid">${list.map(h=>{
      ...
      return `<article class="hydrant-card" data-hydrant-open="${h.id}">
        <div class="hydrant-photo">${h.foto?`<img src="${h.foto}" alt="" loading="lazy" decoding="async">`:ICONS.hydrant}</div>
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v4.1.3 · Migração das Fotos Antigas</div>
          </div>
        </div>
