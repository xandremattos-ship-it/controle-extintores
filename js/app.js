...
let NOTIFICATION_READ_WRITE_PROMISE = Promise.resolve();
...
function enqueueNotificationReadWrite(task){
  const operation = NOTIFICATION_READ_WRITE_PROMISE
    .catch(()=>{})
    .then(task);

  // A fila interna nunca permanece rejeitada, mas a chamada atual
  // continua recebendo o erro para poder tratá-lo normalmente.
  NOTIFICATION_READ_WRITE_PROMISE = operation.catch(()=>{});

  return operation;
}

function isNotificationWriteConflict(error){
...
async function flushPendingNotificationReadIds(){
  ...
  return enqueueNotificationReadWrite(async()=>{
    // A lista é relida somente quando chega a vez desta operação.
    // Assim, cliques rápidos e atualizações automáticas são agrupados.
    ...
    try{
      /*
       * arrayUnion é atômico e idempotente:
       * - cria o documento quando ainda não existe;
       * - adiciona somente IDs ainda ausentes;
       * - não utiliza precondição exists/updateTime;
       * - aceita gravações simultâneas de celular e computador.
       */
      await setDoc(
        stateReference,
        {
          uid:AUTH_USER.uid,
          readIds:arrayUnion(...idsToSend),
          updatedAt:serverTimestamp()
        },
        {merge:true}
      );
      ...
      /*
       * Compatibilidade defensiva para páginas antigas ainda abertas.
       * Após o novo Service Worker assumir o controle, essas condições
       * não devem voltar a acontecer.
       */
      if(isNotificationWriteConflict(error)){
        console.warn(
          'Conflito antigo de notificação neutralizado. Reconsultando o servidor.',
          error
        );
    ...
    idsToSend.forEach(id=>
      NOTIFICATION_REMOTE_READ_IDS.add(String(id))
    );

    const latestPending = loadPendingNotificationReadIds();
    idsToSend.forEach(id=>latestPending.delete(String(id)));
...
async function logout(){
  try{
    ...
    NOTIFICATION_READ_SYNCED_AT = 0;
    NOTIFICATION_READ_WRITE_PROMISE = Promise.resolve();
    NOTIFICATION_READ_SYNC_PROMISE = null;
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v3.2.13.1 · Estabilidade das Notificações</div>
          </div>
        </div>
