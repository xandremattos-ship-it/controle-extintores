...
/* IDs de hidrantes marcados como excluídos na nuvem. */
let DELETED_HYDRANT_IDS = new Set();

/*
  Parte 4 — migração controlada das fotos antigas.
  As fotos são migradas em lotes pequenos para não sobrecarregar
  o navegador, o Firestore ou o Storage.
*/
const LEGACY_PHOTO_MIGRATION_BATCH_SIZE = 10;
let LEGACY_PHOTO_MIGRATION_RUNNING = false;
let PENDING_PHOTO_REFERENCE_REPAIRS = [];

/*
  Parte 5 — otimização final:
  miniaturas leves para cards e limpeza de caches operacionais antigos.
*/
const STORAGE_THUMB_BATCH_SIZE = 15;
const STORAGE_THUMB_MAX_DIMENSION = 360;
let STORAGE_THUMB_OPTIMIZATION_RUNNING = false;

/*
  v4.1.5 — manutenção automática.
  As rotinas técnicas continuam disponíveis internamente, mas deixam
  de poluir a tela normal de Configurações.
*/
const STORAGE_AUTO_LEGACY_BATCH_SIZE = 5;
const STORAGE_AUTO_THUMB_BATCH_SIZE = 8;
const STORAGE_AUTO_INTERVAL_MS = 15 * 60 * 1000;
const STORAGE_AUTO_CONTINUATION_DELAY_MS = 30 * 1000;

let STORAGE_AUTO_MAINTENANCE_RUNNING = false;
let STORAGE_AUTO_MAINTENANCE_TIMER = null;
let STORAGE_AUTO_MAINTENANCE_INTERVAL = null;

/* ============================================================
   PRESENÇA DOS USUÁRIOS — ONLINE E VISTO POR ÚLTIMO
============================================================ */
const USER_PRESENCE_HEARTBEAT_MS = 60 * 1000;
const USER_PRESENCE_ONLINE_WINDOW_MS = 3 * 60 * 1000;
const USER_PRESENCE_UI_REFRESH_MS = 20 * 1000;

let CURRENT_USER_PRESENCE_TIMER = null;
let CURRENT_USER_PRESENCE_WRITE = null;
let CURRENT_USER_PRESENCE_VISIBILITY_HANDLER = null;
let CURRENT_USER_PRESENCE_ONLINE_HANDLER = null;

let USER_PRESENCE_CACHE = new Map();
let USER_PRESENCE_UNSUBSCRIBE = null;
let USER_PRESENCE_UI_TIMER = null;

async function writeCurrentUserPresence(state='ONLINE'){
  if(
    !FIREBASE_READY ||
    !FIREBASE_DB ||
    !AUTH_USER ||
    !userCanEnterSystem(CURRENT_USER_PROFILE) ||
    !navigator.onLine
  ){
    return false;
  }

  if(CURRENT_USER_PRESENCE_WRITE){
    return CURRENT_USER_PRESENCE_WRITE;
  }

  CURRENT_USER_PRESENCE_WRITE = (async()=>{
    const {
      doc,
      setDoc,
      serverTimestamp
    ...
    await setDoc(
      doc(FIREBASE_DB,'userPresence',AUTH_USER.uid),
      {
        uid:AUTH_USER.uid,
    ...
    .catch(error=>{
      console.warn(
        'Não foi possível atualizar a presença do usuário.',
        error
      );
      return false;
    })
    .finally(()=>{
      CURRENT_USER_PRESENCE_WRITE = null;
    });

  return CURRENT_USER_PRESENCE_WRITE;
}

function clearCurrentUserPresenceTracking(){
  if(CURRENT_USER_PRESENCE_TIMER){
    clearInterval(CURRENT_USER_PRESENCE_TIMER);
    CURRENT_USER_PRESENCE_TIMER = null;
  }

  if(CURRENT_USER_PRESENCE_VISIBILITY_HANDLER){
    document.removeEventListener(
      'visibilitychange',
      CURRENT_USER_PRESENCE_VISIBILITY_HANDLER
    );
    CURRENT_USER_PRESENCE_VISIBILITY_HANDLER = null;
  }

  if(CURRENT_USER_PRESENCE_ONLINE_HANDLER){
    window.removeEventListener(
      'online',
      CURRENT_USER_PRESENCE_ONLINE_HANDLER
    );
    CURRENT_USER_PRESENCE_ONLINE_HANDLER = null;
  }
}

function startCurrentUserPresence(){
  clearCurrentUserPresenceTracking();

  writeCurrentUserPresence('ONLINE');

  CURRENT_USER_PRESENCE_TIMER = setInterval(()=>{
    if(
...
async function stopCurrentUserPresence({
  markOffline=false
}={}){
  clearCurrentUserPresenceTracking();

  if(markOffline){
    await writeCurrentUserPresence('OFFLINE');
  }
}

function formatPresenceElapsed(timestamp,now=Date.now()){
  if(!timestamp) return '';
...
function userPresenceInfo(user,now=Date.now()){
  const presence =
    USER_PRESENCE_CACHE.get(String(user?.uid || '')) ||
    null;

  const lastSeenTimestamp = notificationTimestamp(
    presence?.lastSeenAt ||
    presence?.updatedAt
  );

  const activeAccount =
    normalizeUserStatus(user)==='ATIVO' &&
    user?.active!==false;
  ...
  const online =
    ...
    !!lastSeenTimestamp &&
    now-lastSeenTimestamp<=USER_PRESENCE_ONLINE_WINDOW_MS;

  if(online){
    return {
      state:'online',
      headline:'Online agora',
      detail:'Utilizando o sistema neste momento',
      exact:lastSeenTimestamp
        ? formatUserTimestamp(presence.lastSeenAt || presence.updatedAt)
        : ''
    };
  }
  ...
  if(lastSeenTimestamp){
    return {
      state:'offline',
      headline:`Visto por último ${formatPresenceElapsed(
        lastSeenTimestamp,
        now
      )}`,
      detail:formatUserTimestamp(
        presence.lastSeenAt || presence.updatedAt
      ),
      exact:formatUserTimestamp(
        presence.lastSeenAt || presence.updatedAt
      )
    };
  }
  ...
  return {
    state:'unknown',
    headline:'Sem registro de acesso',
    detail:'Será registrado no próximo acesso ao sistema',
    exact:''
  };
}

function userPresenceHtml(user){
  const presence = userPresenceInfo(user);

  return `
    <div
      class="user-presence ${presence.state}"
      data-user-presence="${escapeHtml(String(user.uid))}"
      title="${escapeHtml(presence.exact || presence.detail)}">

      <span class="user-presence-dot"></span>

      <div>
        <b data-user-presence-headline>
          ${escapeHtml(presence.headline)}
        </b>

        <small data-user-presence-detail>
...
function updateUsersPresenceUi(){
  ...
  document
    .querySelectorAll('[data-user-presence]')
    .forEach(container=>{
      const uid = String(container.dataset.userPresence || '');
      const user = USERS_CACHE.find(item=>
        String(item.uid)===uid
      );

      if(!user) return;

      const presence = userPresenceInfo(user,now);
      ...
      container.classList.add(presence.state);
      container.title = presence.exact || presence.detail;

      const headline =
        container.querySelector(
          '[data-user-presence-headline]'
        );

      const detail =
        container.querySelector(
          '[data-user-presence-detail]'
        );

      if(headline){
        headline.textContent = presence.headline;
      }
  ...
  const onlineCount = USERS_CACHE.filter(user=>
    userPresenceInfo(user,now).state==='online'
  ).length;

  const onlineCounter =
    document.getElementById('users-online-count');

  if(onlineCounter){
    onlineCounter.textContent = String(onlineCount);
  }
}

async function loadUserPresenceSnapshot(){
  if(!can('users') || !FIREBASE_DB){
    USER_PRESENCE_CACHE = new Map();
    return USER_PRESENCE_CACHE;
  }
  ...
  const snapshot = await getDocsFromServer(
    collection(FIREBASE_DB,'userPresence')
  );
...
function stopUsersPresenceRealtime(){
  if(USER_PRESENCE_UNSUBSCRIBE){
    USER_PRESENCE_UNSUBSCRIBE();
    USER_PRESENCE_UNSUBSCRIBE = null;
  }

  if(USER_PRESENCE_UI_TIMER){
    clearInterval(USER_PRESENCE_UI_TIMER);
    USER_PRESENCE_UI_TIMER = null;
  }
}

async function startUsersPresenceRealtime(){
  stopUsersPresenceRealtime();
  if(!can('users') || !FIREBASE_DB) return;

  const refreshPresence = async()=>{
    if(!navigator.onLine || STATE.view!=='users'){
      updateUsersPresenceUi();
      return;
    }
    try{
      await loadUserPresenceSnapshot();
      updateUsersPresenceUi();
    }catch(error){
      console.warn('Atualização da presença adiada.',error?.message || error);
    }
  };

  await refreshPresence();
  USER_PRESENCE_UI_TIMER = setInterval(refreshPresence,USER_PRESENCE_UI_REFRESH_MS);
}
...
function isLargeOperationalCacheKey(key){
...
function saveLocalKey(key, val){
  ...
  /*
    Coleções operacionais grandes ficam somente no IndexedDB.
    Isso evita serializar centenas de registros (e fotos) no localStorage,
    eliminando estouro de cota e reduzindo o tempo de abertura do sistema.
  */
  if(isLargeOperationalCacheKey(key)){
    return true;
  }
...
function pruneLegacyOperationalLocalStorage(){
  try{
    Object.keys(localStorage)
      .filter(storageKey=>{
        if(!storageKey.startsWith('extintores:')) return false;

        const rawKey = storageKey.slice('extintores:'.length);
        return isLargeOperationalCacheKey(rawKey);
      })
      .forEach(storageKey=>{
        localStorage.removeItem(storageKey);
      });
    ...
    console.info(
      'Limpeza do cache legado ignorada:',
      error?.message || error
    );
  }
}
...
async function loadCollectionData(collectionName,localKey,{serverOnly=false}={}){
  const scopedLocalKey = hasAllUsinasAccess()
    ? localKey
    : `${localKey}:${AUTH_USER?.uid || 'user'}:${currentUserUsina()}`;

  if(collectionName==='hydrants'){
    const deletedIds = loadLocalKey('deletedHydrantIds',[]);
    DELETED_HYDRANT_IDS = new Set(Array.isArray(deletedIds) ? deletedIds.map(String) : []);
  }

  const indexedValue = await readOfflineValue(scopedLocalKey,null);
  const localValue = indexedValue ?? loadLocalKey(scopedLocalKey,[]);
  const safeLocalValue = collectionName==='hydrants' && Array.isArray(localValue)
    ? localValue.filter(item=>!DELETED_HYDRANT_IDS.has(String(item?.id || '')))
    : localValue;

  if(!FIREBASE_READY || !FIREBASE_DB) return safeLocalValue;

  try{
    const {collection,getDocs,getDocsFromServer,query,where} =
      await import('https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js');
    ...
    const snap = serverOnly ? await getDocsFromServer(sourceQuery) : await getDocs(sourceQuery);
    const rawItems = snap.docs.map(d=>({...d.data(),id:d.data().id || d.id}));

    if(collectionName==='hydrants'){
      DELETED_HYDRANT_IDS = new Set(rawItems.filter(item=>item?.deleted===true).map(item=>String(item.id)));
      saveLocalKey('deletedHydrantIds',[...DELETED_HYDRANT_IDS]);
    }

    const items = rawItems.filter(item=>item?.deleted!==true);
    saveLocalKey(scopedLocalKey,items);
    await cacheOfflineValue(scopedLocalKey,items);
    return items;
  }catch(error){
    console.error('Scoped collection load error',collectionName,error);
    return safeLocalValue;
  }
}
...
let REMOTE_DATA_REFRESH_PROMISE = null;
let LAST_REMOTE_DATA_REFRESH_AT = 0;
const REMOTE_DATA_REFRESH_MIN_INTERVAL_MS = 30 * 1000;

async function loadCachedApplicationState(){
  const [
    config,
    extinguishers,
    inspections,
    ...
    readOfflineValue('config',null),
    readOfflineValue(scopedOfflineKey('extinguishers'),null),
    readOfflineValue(scopedOfflineKey('inspections'),null),
    readOfflineValue(scopedOfflineKey('hydrants'),null),
    readOfflineValue(scopedOfflineKey('hydrantInspections'),null)
  ]);
  ...
  STATE.extinguishers = extinguishers;
  STATE.inspections = inspections;
  STATE.hydrants = hydrants;
  STATE.hydrantInspections = hydrantInspections;

  if(typeof reconcilePendingOperationsIntoState==='function'){
    await reconcilePendingOperationsIntoState();
  }

  normalizeStoragePhotoReferencesInState();

  STATE.loaded = true;
  return true;
}

function scheduleCurrentOfflineSessionCache(){
  const persist = ()=>{
    cacheCurrentOfflineSession().catch(error=>{
      console.warn(
        'Cache de sessão em segundo plano não concluído.',
        error
      );
    });
  };
...
async function pullLatestCloudState({
  ...
  if(
    ...
    if(manual){
      showToast(
        'Não foi possível buscar dados da nuvem agora.',
        'err'
      );
    }
  ...
  if(
    !force &&
    now-LAST_REMOTE_DATA_REFRESH_AT <
      REMOTE_DATA_REFRESH_MIN_INTERVAL_MS
  ){
    if(manual){
      showToast(
        'Dados já estão atualizados neste aparelho.',
        'ok'
      );
    }
  ...
  if(REMOTE_DATA_REFRESH_PROMISE){
    if(manual){
      showToast(
        'Atualização dos dados já está em andamento.'
      );
    }

    return REMOTE_DATA_REFRESH_PROMISE;
  }

  REMOTE_DATA_REFRESH_PROMISE = (async()=>{
    try{
      const [
        config,
        extinguishers,
      ...
      STATE.hydrants =
        Array.isArray(hydrants)
          ? hydrants
          : [];

      STATE.hydrantInspections =
        Array.isArray(hydrantInspections)
          ? hydrantInspections
          : [];

      /*
        Alterações locais ainda pendentes sempre são reaplicadas
        depois da leitura da nuvem.
      */
      if(
        typeof reconcilePendingOperationsIntoState===
          'function'
      ){
        await reconcilePendingOperationsIntoState();
      }
      ...
      LAST_REMOTE_DATA_REFRESH_AT = Date.now();
      STATE.loaded = true;

      scheduleCurrentOfflineSessionCache();
      ...
      if(manual){
        showToast(
          'Dados atualizados entre navegador, aplicativo e nuvem.',
          'ok'
        );
      }

      scheduleAutomaticStorageMaintenance({
        delay:1800,
        reason:manual
          ? 'manual-pull'
          : 'cloud-pull'
      ...
      console.error(
        'Atualização dos dados da nuvem falhou:',
        error
      );

      if(manual){
        showToast(
          'Não foi possível concluir a atualização dos dados.',
          'err'
        );
      }
      ...
      REMOTE_DATA_REFRESH_PROMISE = null;
    }
  })();
...
async function loadAll(){
  ...
  setTimeout(()=>{
    repairStoragePhotoReferenceDrift()
      .catch(error=>{
        console.warn(
          'Reparo de referência de foto será tentado novamente.',
          error?.message || error
        );
      });

    cleanupObsoleteOperationalCaches({
      notify:false
    }).catch(error=>{
      console.warn(
        'Limpeza automática de cache será repetida depois.',
        error?.message || error
      );
    });
  },700);
  ...
  /*
    As coleções já foram salvas no IndexedDB durante a leitura.
    O restante da sessão é persistido sem bloquear a entrada do usuário.
  */
  scheduleCurrentOfflineSessionCache();
  pruneLegacyOperationalLocalStorage();
}
...
async function persistHydrantDeletion(hydrantId,hydrant=null){
  scheduleOfflineSnapshotCache(
    scopedOfflineKey('hydrants'),
    STATE.hydrants
  );
  ...
  await enqueueOfflineOperation({
    ...
    storagePaths:[
      hydrant?.fotoStoragePath,
      hydrant?.fotoThumbStoragePath
...
function loadImageFileForStorage(file){
  return new Promise((resolve,reject)=>{
    ...
    image.onerror = error=>{
      URL.revokeObjectURL(objectUrl);
      reject(error || new Error('IMAGE_DECODE_FAILED'));
    };
...
function canvasToStorageBlob(canvas,type='image/webp',quality=.80){
  return new Promise((resolve,reject)=>{
    canvas.toBlob(blob=>{
      if(blob){
        resolve(blob);
...
async function prepareExtinguisherPhotoForStorage(file){
  ...
  const image = await loadImageFileForStorage(file);
  ...
  let blob = await canvasToStorageBlob(canvas,'image/webp',.80);

  if(blob.size>2.6*1024*1024){
    blob = await canvasToStorageBlob(canvas,'image/webp',.66);
  }
  ...
  return {
    ...
    optimizedBytes:Number(blob.size || 0)
  };
}

function extinguisherPhotoStorageErrorMessage(error){
  ...
  if(message.includes('IMAGE_TOO_LARGE')){
    return 'A foto original é muito grande. Use uma imagem de até 20 MB.';
  }

  if(message.includes('COMPRESSED_IMAGE_TOO_LARGE')){
    return 'A foto continuou muito grande após a otimização. Escolha outra imagem.';
  }

  if(code.includes('unauthorized')){
    return 'O Firebase Storage não autorizou esta foto. Publique as regras Storage da v4.1.1.';
  }

  if(code.includes('quota-exceeded')){
    return 'O Firebase Storage informou limite de cota.';
  }

  if(code.includes('retry-limit-exceeded') || message.toLowerCase().includes('network')){
    return 'A foto não pôde ser enviada por problema de conexão.';
  }

  return 'Não foi possível enviar a foto para o Firebase Storage.';
}

async function uploadExtinguisherPhotoToStorage(extinguisher,preparedPhoto){
  if(!preparedPhoto) return null;

  if(!navigator.onLine){
    throw new Error('PHOTO_STORAGE_REQUIRES_ONLINE');
  }

  if(!FIREBASE_STORAGE || !AUTH_USER){
  ...
  const path = `extintores/${String(extinguisher.id)}/foto.${preparedPhoto.extension}`;
  const storageReference = ref(FIREBASE_STORAGE,path);
  ...
  await uploadBytes(
    ...
    preparedPhoto.blob,
    {
      ...
      customMetadata:{
        usina:String(extinguisher.usina || ''),
        equipmentId:String(extinguisher.id),
        kind:'extinguisher-photo',
        uploadedBy:String(AUTH_USER.uid),
        appVersion:'4.1.5'
      }
    }
  );

  const rawUrl = await getDownloadURL(storageReference);
  ...
  const thumbnail =
    await tryUploadThumbnailForPreparedPhoto(
      extinguisher,
      'extinguisher',
      preparedPhoto
    );
  ...
  return {
    ...
    bytes:preparedPhoto.optimizedBytes,
    thumbnail
  };
}

async function prepareHydrantPhotoForStorage(file){
  return prepareExtinguisherPhotoForStorage(file);
}

function hydrantPhotoStorageErrorMessage(error){
  const code = String(error?.code || '');
  const message = String(error?.message || '');
  ...
  if(message.includes('PHOTO_STORAGE_REQUIRES_ONLINE')){
    return 'Para salvar uma nova foto do hidrante, conecte este aparelho à internet.';
  }

  if(message.includes('PHOTO_STORAGE_UNAVAILABLE')){
    return 'Firebase Storage ainda não está disponível neste aparelho.';
  }

  if(code.includes('unauthorized')){
    return 'O Firebase Storage não autorizou esta foto. Publique as regras Storage da v4.1.2.';
  }
  ...
  return 'Não foi possível enviar a foto do hidrante para o Firebase Storage.';
}

async function uploadHydrantPhotoToStorage(hydrant,preparedPhoto){
  if(!preparedPhoto) return null;
  ...
  const path =
    `hidrantes/${String(hydrant.id)}/foto.${preparedPhoto.extension}`;
  ...
  await uploadBytes(
    ...
    preparedPhoto.blob,
    {
      ...
      cacheControl:'public,max-age=300',
      customMetadata:{
        usina:String(hydrant.usina || ''),
        equipmentId:String(hydrant.id),
        kind:'hydrant-photo',
        uploadedBy:String(AUTH_USER.uid),
        appVersion:'4.1.5'
      }
...
function isLegacyEmbeddedPhoto(value){
  return (
    typeof value==='string' &&
...
function estimateLegacyPhotoBytes(value){
  if(!isLegacyEmbeddedPhoto(value)) return 0;
...
function legacyPhotoDataUrlToBlob(dataUrl){
  if(!isLegacyEmbeddedPhoto(dataUrl)){
    throw new Error('INVALID_LEGACY_PHOTO');
  }
  ...
  return new Blob(
    [bytes],
    {type:mimeMatch[1].toLowerCase()}
  );
}

function legacyPhotoMigrationInventory(){
  const extinguisherPending = STATE.extinguishers.filter(item=>
    isLegacyEmbeddedPhoto(item?.foto) &&
    !item?.fotoStoragePath
  );

  const hydrantPending = STATE.hydrants.filter(item=>
    isLegacyEmbeddedPhoto(item?.foto) &&
    !item?.fotoStoragePath
  );

  const extinguisherStorage = STATE.extinguishers.filter(item=>
    !!item?.fotoStoragePath
  );

  const hydrantStorage = STATE.hydrants.filter(item=>
    !!item?.fotoStoragePath
  );

  const pending = [
    ...extinguisherPending.map(record=>({
      type:'extinguisher',
      collection:'extinguishers',
      record
    })),
    ...hydrantPending.map(record=>({
      type:'hydrant',
      collection:'hydrants',
      record
    }))
  ...
  const estimatedBytes = pending.reduce(
    (sum,item)=>
      sum+estimateLegacyPhotoBytes(item.record?.foto),
    0
  );
...
function storagePhotoMigrationStatsHtml(){
  const inventory = legacyPhotoMigrationInventory();

  return `
    <div class="storage-migration-stats">
      <div class="storage-migration-stat warning">
        <span>Pendentes</span>
        <b>${inventory.pending.length}</b>
        <small>${formatMediaBytes(inventory.estimatedBytes)} aproximados</small>
      </div>

      <div class="storage-migration-stat">
        <span>Extintores antigos</span>
        <b>${inventory.extinguisherPending.length}</b>
        <small>aguardando migração</small>
      </div>

      <div class="storage-migration-stat">
        <span>Hidrantes antigos</span>
        <b>${inventory.hydrantPending.length}</b>
        <small>aguardando migração</small>
      </div>

      <div class="storage-migration-stat success">
        <span>Já no Storage</span>
        <b>${inventory.extinguisherStorage.length+inventory.hydrantStorage.length}</b>
        <small>extintores + hidrantes</small>
      </div>
    </div>
  `;
}

function refreshStorageMigrationPanel(){
  const stats = document.getElementById('storage-migration-stats');

  if(stats){
    stats.innerHTML = storagePhotoMigrationStatsHtml();
  }

  const inventory = legacyPhotoMigrationInventory();

  const button = document.getElementById('storage-migrate-batch');

  if(button){
    button.disabled =
      LEGACY_PHOTO_MIGRATION_RUNNING ||
      inventory.pending.length===0;

    button.textContent = inventory.pending.length
      ? `Migrar próximo lote · até ${Math.min(
          LEGACY_PHOTO_MIGRATION_BATCH_SIZE,
          inventory.pending.length
        )} foto(s)`
      : 'Migração concluída';
  }

  const summary = document.getElementById('storage-migration-summary');

  if(summary){
    summary.textContent = inventory.pending.length
      ? `${inventory.pending.length} foto(s) antiga(s) ainda permanecem incorporadas ao Firestore.`
      : 'Nenhuma foto antiga incorporada permanece pendente.';
  }
}

function setStorageMigrationProgress({
  visible=true,
  current=0,
  total=0,
  label=''
}={}){
  const container = document.getElementById('storage-migration-progress');
  if(!container) return;
  ...
  const bar = container.querySelector('[data-storage-migration-bar]');
  if(bar) bar.style.width = `${percentage}%`;

  const count = container.querySelector('[data-storage-migration-count]');
  if(count) count.textContent = total ? `${current}/${total}` : '';

  const text = container.querySelector('[data-storage-migration-label]');
  if(text) text.textContent = label;
}

function normalizeStoragePhotoReferencesInState(){
  const repairs = [];

  const inspect = (collection,records)=>{
    records.forEach(record=>{
      if(
        record?.fotoStorageUrl &&
        record?.fotoStoragePath &&
        isLegacyEmbeddedPhoto(record?.foto)
      ){
        record.foto = record.fotoStorageUrl;

        repairs.push({
          collection,
          id:String(record.id),
          url:String(record.fotoStorageUrl)
        });
      }
    });
  };

  inspect('extinguishers',STATE.extinguishers);
  inspect('hydrants',STATE.hydrants);

  PENDING_PHOTO_REFERENCE_REPAIRS = repairs;
...
async function repairStoragePhotoReferenceDrift(){
  if(
    !navigator.onLine ||
    !FIREBASE_READY ||
    !FIREBASE_DB ||
    !PENDING_PHOTO_REFERENCE_REPAIRS.length
  ){
    return 0;
  }

  const repairs = [...PENDING_PHOTO_REFERENCE_REPAIRS];
  PENDING_PHOTO_REFERENCE_REPAIRS = [];
  ...
  for(const repair of repairs){
    try{
      await setDoc(
        doc(FIREBASE_DB,repair.collection,repair.id),
        {foto:repair.url},
        {merge:true}
      );
      ...
      console.warn(
        'Referência de foto migrada não pôde ser reparada agora.',
        repair.collection,
        repair.id,
        error?.message || error
      );

      PENDING_PHOTO_REFERENCE_REPAIRS.push(repair);
    }
  }

  if(repaired){
    publishCloudSyncSignal().catch(()=>{});
  }

  return repaired;
}

async function removeStorageObjectQuietly(path){
  if(!path || !FIREBASE_STORAGE) return;
  ...
  try{
    ...
    await deleteObject(
      ref(FIREBASE_STORAGE,String(path))
    );
  }catch(error){
    if(error?.code !== 'storage/object-not-found'){
      console.warn(
        'Arquivo temporário da migração não pôde ser removido.',
        path,
        error?.message || error
      );
    }
...
async function migrateLegacyPhotoItem(item){
  ...
  const originalFoto = record.foto;
  let uploadedPhoto = null;

  try{
    const sourceBlob = legacyPhotoDataUrlToBlob(originalFoto);

    const preparedPhoto = item.type==='hydrant'
      ? await prepareHydrantPhotoForStorage(sourceBlob)
      : await prepareExtinguisherPhotoForStorage(sourceBlob);

    uploadedPhoto = item.type==='hydrant'
      ? await uploadHydrantPhotoToStorage(record,preparedPhoto)
      : await uploadExtinguisherPhotoToStorage(record,preparedPhoto);

    if(!uploadedPhoto?.url){
      throw new Error('MIGRATION_UPLOAD_FAILED');
    }
    ...
    const migrationPatch = {
      foto:uploadedPhoto.url,
      fotoStorageUrl:uploadedPhoto.url,
      fotoStoragePath:uploadedPhoto.path,
      fotoStorageProvider:'firebase-storage',
      fotoStorageContentType:uploadedPhoto.contentType,
      fotoStorageBytes:uploadedPhoto.bytes,
      fotoStorageWidth:uploadedPhoto.width,
      fotoStorageHeight:uploadedPhoto.height,
      fotoStorageUpdatedAt:new Date().toISOString(),
      fotoStorageVersion:1,
      fotoStorageMigrated:true,
      fotoStorageMigratedAt:serverTimestamp(),
      fotoStorageMigratedBy:AUTH_USER?.uid || '',
      ...(uploadedPhoto.thumbnail
        ? {
            fotoThumbUrl:
              uploadedPhoto.thumbnail.url,
            fotoThumbStoragePath:
              uploadedPhoto.thumbnail.path,
            fotoThumbContentType:
              uploadedPhoto.thumbnail.contentType,
            fotoThumbBytes:
              uploadedPhoto.thumbnail.bytes,
            fotoThumbWidth:
              uploadedPhoto.thumbnail.width,
            fotoThumbHeight:
              uploadedPhoto.thumbnail.height,
            fotoThumbUpdatedAt:
    ...
    await setDoc(
      doc(FIREBASE_DB,item.collection,String(record.id)),
      migrationPatch,
      {merge:true}
    );
    ...
    Object.assign(
      ...
      {
        ...migrationPatch,
        fotoStorageMigratedAt:new Date().toISOString()
      }
    );
    ...
    return {
      ...
      type:item.type,
      id:String(record.id),
      bytesBefore:estimateLegacyPhotoBytes(originalFoto),
      bytesAfter:uploadedPhoto.bytes
    };
  }catch(error){
    if(uploadedPhoto?.path){
      await removeStorageObjectQuietly(uploadedPhoto.path);
    }
...
async function migrateLegacyPhotoBatch(button){
  if(LEGACY_PHOTO_MIGRATION_RUNNING) return;
  ...
  if(
    ...
    showToast(
      'A migração precisa de conexão com a internet e Firebase Storage disponível.',
      'err'
    );
    return;
  }
  ...
  if(queue.length){
    showToast(
      'Existem alterações aguardando sincronização. Clique em Sincronizar e, quando a fila estiver vazia, inicie a migração.',
      'err'
    );
    return;
  }
  ...
  const batch = inventory.pending.slice(
    0,
    LEGACY_PHOTO_MIGRATION_BATCH_SIZE
  );

  if(!batch.length){
    refreshStorageMigrationPanel();
    showToast('Não existem fotos antigas pendentes de migração.','ok');
    return;
  }

  const confirmed = await askSystemConfirmation({
    title:'Migrar fotos antigas',
    message:
      `Este lote migrará até ${batch.length} foto(s). `+
      'Cada foto só será retirada do Firestore depois que o envio ao Storage for confirmado.',
  ...
  LEGACY_PHOTO_MIGRATION_RUNNING = true;
  refreshStorageMigrationPanel();
  ...
  setStorageMigrationProgress({
    visible:true,
    current:0,
    total:batch.length,
    label:'Preparando migração...'
  });

  for(let index=0; index<batch.length; index++){
    const item = batch[index];

    const equipmentLabel = item.type==='hydrant'
      ? `Hidrante #${item.record.numero || item.record.id}`
      : `Extintor #${item.record.numero || item.record.id}`;

    setStorageMigrationProgress({
      visible:true,
      current:index,
      total:batch.length,
      label:`Migrando ${equipmentLabel}...`
    });

    try{
      const result = await migrateLegacyPhotoItem(item);

      if(result?.migrated){
        migrated++;
        bytesBefore += Number(result.bytesBefore || 0);
        bytesAfter += Number(result.bytesAfter || 0);
      }
    }catch(error){
      failed++;

      console.error(
        'Falha na migração de foto antiga:',
        item.type,
        item.record?.id,
        error
      );
  ...
  if(migrated){
    await publishCloudSyncSignal().catch(()=>{});
  }

  LEGACY_PHOTO_MIGRATION_RUNNING = false;
  refreshStorageMigrationPanel();
  ...
  setStorageMigrationProgress({
    ...
    total:batch.length,
    label:failed
      ? `${migrated} migrada(s) · ${failed} não migrada(s)`
      : `${migrated} foto(s) migrada(s) com sucesso`
  });

  if(failed){
    showToast(
      `${migrated} foto(s) migrada(s). ${failed} permaneceram intactas para nova tentativa.`,
      'err'
    );
  }else{
    showToast(
      `${migrated} foto(s) migrada(s). Redução aproximada deste lote: ${formatMediaBytes(savedBytes)}.`,
      'ok'
    );
  }

  setTimeout(()=>{
    if(!LEGACY_PHOTO_MIGRATION_RUNNING){
      setStorageMigrationProgress({visible:false});
    }
  },5000);
}

function equipmentCardPhotoSource(record){
  return (
    record?.fotoThumbUrl ||
    record?.foto ||
    ''
...
async function prepareStorageThumbnail(fileOrBlob){
  if(
    !fileOrBlob ||
    !String(fileOrBlob.type || '').startsWith('image/')
  ){
    throw new Error('INVALID_IMAGE');
  }

  const image =
    await loadImageFileForStorage(
      fileOrBlob
    );
  ...
  const ratio =
    Math.min(
      1,
      STORAGE_THUMB_MAX_DIMENSION/width,
      STORAGE_THUMB_MAX_DIMENSION/height
    );
...
async function uploadEquipmentThumbnailToStorage(
  record,
  type,
  preparedThumbnail
){
  ...
  const path =
    `${folder}/${String(record.id)}/thumb.webp`;
  ...
  return {
    ...
    contentType:
      preparedThumbnail.contentType,
    width:
      preparedThumbnail.width,
    height:
      preparedThumbnail.height,
    bytes:
      preparedThumbnail.optimizedBytes
  };
}

async function tryUploadThumbnailForPreparedPhoto(
  record,
  type,
  preparedPhoto
){
  try{
    const thumbnail =
      await prepareStorageThumbnail(
        preparedPhoto.blob
      );

    return await uploadEquipmentThumbnailToStorage(
      record,
      type,
      thumbnail
    );
    ...
    console.warn(
      'Miniatura será gerada posteriormente pela otimização final.',
      type,
      record?.id,
      error?.message || error
    );
...
function applyThumbnailMetadata(
  target,
  thumbnail
){
  ...
  target.fotoThumbStoragePath =
    thumbnail.path;

  target.fotoThumbContentType =
    thumbnail.contentType;

  target.fotoThumbBytes =
    thumbnail.bytes;

  target.fotoThumbWidth =
    thumbnail.width;

  target.fotoThumbHeight =
    thumbnail.height;

  target.fotoThumbUpdatedAt =
    new Date().toISOString();

  target.fotoThumbVersion = 1;
}

function finalMediaOptimizationInventory(){
  const extStorage =
    STATE.extinguishers.filter(item=>
      !!item?.fotoStoragePath &&
      !!item?.foto
    );

  const hydrantStorage =
    STATE.hydrants.filter(item=>
      !!item?.fotoStoragePath &&
      !!item?.foto
    );
  ...
  const thumbReady =
    extStorage.filter(item=>
      !!item?.fotoThumbStoragePath
    ).length+
    hydrantStorage.filter(item=>
      !!item?.fotoThumbStoragePath
    ).length;

  const legacy =
    legacyPhotoMigrationInventory();
  ...
  return {
    ...
    storageTotal:
      extStorage.length+
      hydrantStorage.length,
    thumbPending,
    thumbReady,
    legacyPending:
      legacy.pending.length
  };
}

function finalMediaOptimizationStatsHtml(){
  const inventory =
    finalMediaOptimizationInventory();

  return `
    <div class="storage-final-stats">
      <div class="storage-final-stat">
        <span>Fotos no Storage</span>
        <b>${inventory.storageTotal}</b>
        <small>arquivos principais</small>
      </div>

      <div class="storage-final-stat success">
        <span>Miniaturas prontas</span>
        <b>${inventory.thumbReady}</b>
        <small>cards leves</small>
      </div>

      <div class="storage-final-stat warning">
        <span>Miniaturas pendentes</span>
        <b>${inventory.thumbPending.length}</b>
        <small>aguardando otimização</small>
      </div>

      <div class="storage-final-stat ${
        inventory.legacyPending
          ? 'warning'
          : 'success'
      }">
        <span>Fotos antigas</span>
        <b>${inventory.legacyPending}</b>
        <small>${
          inventory.legacyPending
            ? 'concluir Parte 4'
            : 'migração concluída'
...
function refreshFinalMediaOptimizationPanel(){
  const stats =
  ...
  if(stats){
    stats.innerHTML =
      finalMediaOptimizationStatsHtml();
  }
  ...
  const button =
    document.getElementById(
      'storage-optimize-thumbnails'
    );

  if(button){
    button.disabled =
      STORAGE_THUMB_OPTIMIZATION_RUNNING ||
      inventory.thumbPending.length===0 ||
      inventory.legacyPending>0;

    button.textContent =
      inventory.thumbPending.length
        ? `Otimizar próximo lote · até ${Math.min(
            STORAGE_THUMB_BATCH_SIZE,
            inventory.thumbPending.length
          )} miniatura(s)`
  ...
  const summary =
    document.getElementById(
      'storage-final-summary'
    );

  if(summary){
    if(inventory.legacyPending){
      summary.textContent =
        'Conclua primeiro a migração das fotos antigas da Parte 4.';
    }else if(inventory.thumbPending.length){
      summary.textContent =
        `${inventory.thumbPending.length} equipamento(s) ainda precisam de miniatura leve.`;
    }else{
      summary.textContent =
        'Fotos e miniaturas estão otimizadas. A limpeza final já pode ser executada.';
    }
  }
}

function setFinalMediaProgress({
  visible=true,
  current=0,
  total=0,
  label=''
...
async function fetchPhotoBlobForThumbnail(record){
  const source =
    record?.fotoStorageUrl ||
    record?.foto;
...
async function optimizeThumbnailItem(item){
  const record =
    item?.record;

  if(
    !record ||
    record.fotoThumbStoragePath
  ){
    return {
      optimized:false,
      skipped:true
  ...
  const sourceBlob =
    await fetchPhotoBlobForThumbnail(
      record
    );

  const preparedThumbnail =
    await prepareStorageThumbnail(
      sourceBlob
    );

  const thumbnail =
    await uploadEquipmentThumbnailToStorage(
      record,
      item.type,
      preparedThumbnail
    );
...
async function optimizeStorageThumbnailBatch(button){
  if(STORAGE_THUMB_OPTIMIZATION_RUNNING){
    return;
  }
  ...
  if(
    ...
    showToast(
      'A otimização precisa de conexão com a internet.',
      'err'
    );
  ...
  if(queue.length){
    showToast(
      'Sincronize as alterações pendentes antes de gerar as miniaturas.',
      'err'
    );
  ...
  const batch =
    inventory.thumbPending.slice(
      0,
      STORAGE_THUMB_BATCH_SIZE
    );

  if(!batch.length){
    refreshFinalMediaOptimizationPanel();

    showToast(
      'Todas as miniaturas já estão prontas.',
      'ok'
    );
  ...
  const confirmed =
    await askSystemConfirmation({
      title:'Otimizar imagens dos cards',
      message:
        `Serão criadas até ${batch.length} miniatura(s) leves. `+
        'As fotos principais permanecem no Storage em qualidade maior.',
      confirmLabel:'Otimizar lote',
      cancelLabel:'Cancelar'
    });
  ...
  for(
    ...
    setFinalMediaProgress({
      ...
      current:index,
      total:batch.length,
      label:`Otimizando ${label}...`
    });

    try{
      const result =
        await optimizeThumbnailItem(
          item
        );
  ...
  setFinalMediaProgress({
    ...
    label:
      failed
        ? `${optimized} pronta(s) · ${failed} não criada(s)`
        : `${optimized} miniatura(s) criada(s) · ${formatMediaBytes(totalBytes)}`
  });

  if(failed){
    showToast(
      `${optimized} miniatura(s) criadas. ${failed} poderão ser tentadas novamente.`,
      'err'
    );
  }else{
    showToast(
      `${optimized} miniatura(s) leves criadas com sucesso.`,
      'ok'
    );
  }
...
async function cleanupObsoleteOperationalCaches({
  notify=false
}={}){
  const currentKeys =
    new Set([
  ...
  try{
    Object.keys(localStorage)
      .filter(storageKey=>
        [
          ...
          'extintores:inspections',
          'extintores:hydrants',
          'extintores:hydrantInspections'
        ].some(prefix=>
    ...
    console.warn(
      'Limpeza do localStorage não concluída.',
      error
    );
  }
  ...
  try{
    ...
    console.warn(
      'Limpeza do IndexedDB não concluída.',
      error
    );
  }

  /*
    Regrava somente os caches atuais, já leves e apontando para URLs.
  */
  await Promise.all([
    cacheOfflineValue(
      scopedOfflineKey(
  ...
  if(notify){
    showToast(
      `Limpeza concluída: ${localRemoved} cache(s) local(is) e ${indexedRemoved} cache(s) antigo(s) removidos.`,
      'ok'
    );
  }
...
async function runStorageFinalCleanup(button){
  const inventory =
    finalMediaOptimizationInventory();
  ...
  if(
    ...
    showToast(
      'Conclua primeiro a migração e a criação das miniaturas.',
      'err'
    );
  ...
  if(queue.length){
    showToast(
      'Sincronize as alterações pendentes antes da limpeza final.',
      'err'
    );
  ...
  const confirmed =
    await askSystemConfirmation({
      title:'Executar limpeza final',
      message:
        'O sistema removerá caches operacionais antigos deste aparelho e manterá somente a base atual em IndexedDB. Nenhum cadastro será excluído da nuvem.',
  ...
  try{
    await cleanupObsoleteOperationalCaches({
      notify:true
    });
...
async function imageSourceForPdf(source){
  ...
  try{
    ...
    console.warn('Foto remota indisponível para o PDF.',error);
    return null;
  }
}
...
async function startNotificationReadStateListener(){
  ...
  if(!AUTH_USER || !FIREBASE_DB || !navigator.onLine) return;
  try{
    await syncNotificationReadStateFromCloud({force:true});
  }catch(error){
    console.warn('Sincronização das notificações adiada.',error?.message || error);
  }
}
    ...
    <div class="main">
      <div class="topbar">
        ...
        <div class="topbar-right">
          ...
          <button
            type="button"
            class="btn btn-sm pwa-check-update-btn"
            id="btn-check-update"
            title="Verificar se existe uma nova versão">
            Atualizar sistema
          </button>
...
async function refreshDashboardCollections(){
  if(!navigator.onLine){
    if(
      typeof updateConnectionUi==='function'
    ){
...
async function renderDashboard(){
  ...
  /*
    O painel usa imediatamente os dados que já estão no STATE/IndexedDB.
    A atualização da nuvem ocorre no fluxo geral de sincronização ou
    quando o usuário toca em "Atualizar dados".
  */
...
function cardExt(e){
  ...
  <div class="ext-card status-${st.cls}" data-view-ext="${e.id}" style="cursor:pointer;">
    <div class="ext-photo">
      ${equipmentCardPhotoSource(e)?`<img src="${equipmentCardPhotoSource(e)}" loading="lazy" decoding="async">`:`<div class="no-photo">${ICONS.photo}</div>`}
  ...
  return `
    <article
      ...
      <div class="ext-list-photo">
        ${e.foto
          ? `<img src="${equipmentCardPhotoSource(e)}" alt="Foto do extintor ${escapeHtml(String(e.numero))}" loading="lazy" decoding="async">`
...
function askSystemConfirmation({
  ...
  return new Promise(resolve=>{
    ...
    overlay.id = 'system-confirmation-overlay';
    overlay.className = 'system-confirmation-overlay';

    overlay.innerHTML = `
      <section
        class="system-confirmation-card"
        role="dialog"
        aria-modal="true"
        aria-labelledby="system-confirmation-title">

        <div class="system-confirmation-icon ${
          danger ? 'danger' : ''
        }">
          ${danger ? ICONS.trash : ICONS.checkCircle}
        </div>

        <div class="system-confirmation-copy">
          <h3 id="system-confirmation-title">
            ${escapeHtml(title)}
          </h3>
        ...
        <div class="system-confirmation-actions">
          ...
          <button
            type="button"
            class="btn ${
              danger ? 'btn-danger' : 'btn-accent'
            }"
            id="system-confirmation-ok">
...
async function deleteExtinguisher(id){
  ...
  if(
    !await askSystemConfirmation({
      title:'Excluir extintor',
      message:'O extintor e todo o histórico de inspeções serão removidos.',
...
function openExtinguisherModal(id){
  ...
  let pendingStoragePhoto = null;
  let pendingStoragePreviewUrl = null;
    ...
    <div class="modal ${editing?'':'inspection-form-modal'}" ${editing?'':'style="max-width:780px;"'}>
      ...
      <div class="modal-body">
        <div class="field"><label>Foto do extintor</label>
          <label class="photo-upload" id="extinguisher-photo-upload">
          ...
          <div class="storage-photo-note" id="extinguisher-photo-note">
            ${e.fotoStoragePath
              ? 'Foto armazenada no Firebase Storage.'
              : e.foto
                ? 'Foto antiga preservada. Ela só irá ao Storage quando for substituída.'
                : 'Fotos novas são otimizadas e enviadas ao Firebase Storage.'}
          </div>
        </div>
        <div class="field-row">
  ...
  function clearPendingStoragePreview(){
    if(pendingStoragePreviewUrl){
      URL.revokeObjectURL(pendingStoragePreviewUrl);
      pendingStoragePreviewUrl = null;
    }
  }
  ...
  function bindFotoInput(){
    document.getElementById('f-foto')?.addEventListener('change', async ev=>{
      ...
      const upload = document.getElementById('extinguisher-photo-upload');
      const note = document.getElementById('extinguisher-photo-note');

      try{
        if(note){
          note.textContent = 'Otimizando a foto...';
          note.classList.remove('ok','error');
        }

        const prepared = await prepareExtinguisherPhotoForStorage(file);

        clearPendingStoragePreview();
        pendingStoragePhoto = prepared;
        pendingStoragePreviewUrl = URL.createObjectURL(prepared.blob);

        if(upload){
          upload.innerHTML = `
            <img src="${pendingStoragePreviewUrl}" alt="Prévia da nova foto">
            <input type="file" accept="image/*" id="f-foto">
          `;
        }

        if(note){
          note.textContent = `Nova foto pronta para o Storage · ${formatMediaBytes(prepared.optimizedBytes)}`;
          note.classList.add('ok');
        }

        bindFotoInput();
      }catch(error){
        console.error('Erro ao preparar foto do extintor:',error);
        pendingStoragePhoto = null;

        if(note){
          note.textContent = extinguisherPhotoStorageErrorMessage(error);
          note.classList.add('error');
        }

        showToast(
          extinguisherPhotoStorageErrorMessage(error),
          'err'
        );
      }
    });
  ...
  document.getElementById('m-save').addEventListener('click', async ()=>{
    ...
    try{
      const commitPendingStoragePhoto = async()=>{
        if(!pendingStoragePhoto) return true;

        if(!navigator.onLine){
          showToast(
            'Para salvar uma nova foto no Storage, conecte este aparelho à internet.',
            'err'
          );
          return false;
        }
        ...
        try{
          const uploadedPhoto =
            await uploadExtinguisherPhotoToStorage(
              e,
              pendingStoragePhoto
            );

          if(uploadedPhoto){
            e.foto = uploadedPhoto.url;
            e.fotoStorageUrl = uploadedPhoto.url;
            e.fotoStoragePath = uploadedPhoto.path;
            e.fotoStorageProvider = 'firebase-storage';
            e.fotoStorageContentType = uploadedPhoto.contentType;
            e.fotoStorageBytes = uploadedPhoto.bytes;
            e.fotoStorageWidth = uploadedPhoto.width;
            e.fotoStorageHeight = uploadedPhoto.height;
            e.fotoStorageUpdatedAt = new Date().toISOString();
            e.fotoStorageVersion = 1;
          ...
          showToast(
            extinguisherPhotoStorageErrorMessage(photoError),
            'err'
          );
          return false;
        }
      ...
      if(!editing){
        ...
        if(!await commitPendingStoragePhoto()){
          saveBtn.disabled = false;
          saveBtn.textContent = originalText;
          return;
        }
      ...
      if(editing && !await commitPendingStoragePhoto()){
        saveBtn.disabled = false;
        saveBtn.textContent = originalText;
        return;
      }
      ...
      <div class="modal-body">
        <div style="display:flex;gap:14px;align-items:center;margin-bottom:16px;">
          <div style="width:82px;height:82px;border-radius:10px;overflow:hidden;background:var(--panel-2);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            ${equipmentCardPhotoSource(e) ? `<img src="${equipmentCardPhotoSource(e)}" style="width:100%;height:100%;object-fit:cover;" loading="lazy" decoding="async">` : ICONS.photo}
          </div>
    ...
    button.addEventListener('click',async()=>{
      ...
      const confirmed =
        await askSystemConfirmation({
          ...
          message:`Excluir definitivamente a inspeção de ${fmtDate(inspection.date)}?`,
          confirmLabel:'Excluir',
          danger:true
        });
...
function generateGeneralPdf(){
  ...
  try{
    ...
    /*
      Bloco final com legenda, assinaturas e aprovações.
      O espaço vertical foi ampliado para permitir assinatura manual.
    */
    ...
    const legendItems = [
      {
        label:'CONFORME / VÁLIDO',
        color:[28,125,75]
      },
      {
        label:'ALERTA / ATÉ 30 DIAS',
        color:[161,109,9]
      },
      {
        label:'NÃO CONFORME / VENCIDO / DESATIVADO',
        color:[196,48,31]
      },
      {
        label:'ESTOQUE',
    ...
    /*
      Seção final de assinatura.
      A data de emissão já aparece no cabeçalho do relatório.
    */
    let signatureTitleY = finalY + 58;

    if(signatureTitleY > pageHeight-172){
      doc.addPage();
      drawHeader();
      signatureTitleY = 92;
    }
    ...
    /*
      Campo centralizado e com espaço ampliado
      para assinatura manual, nome e carimbo.
    */
...
async function generateIndividualPdf(e){
  if(!window.jspdf || !window.jspdf.jsPDF){
    showToast("Biblioteca de PDF não carregou. Verifique sua conexão e recarregue a página.","err");
    return;
  }
  ...
  try{
    ...
    if(e.foto){
      try{
        const pdfPhoto = await imageSourceForPdf(e.foto);
        if(pdfPhoto){
          doc.addImage(pdfPhoto,'JPEG',marginX,y,118,118);
        }else{
        ...
        doc.text('Foto indisponível',marginX+59,y+62,{align:'center'});
      }
    }else{
      doc.setFillColor(244,246,248);
      doc.roundedRect(marginX,y,118,118,5,5,'F');
  ...
  try{
    ...
    content.innerHTML = `
      ...
      <div class="users-list-head">
        ...
        <div class="users-live-summary">
          <span>
            <i></i>
            <b id="users-online-count">${
              managedUsers.filter(user=>
                userPresenceInfo(user).state==='online'
              ).length
            }</b>
            online
          </span>
  ...
  document.getElementById('approval-reject')?.addEventListener('click',async event=>{
    if(
      !await askSystemConfirmation({
        ...
        message:`Rejeitar a solicitação de "${user.name || user.email}"?`,
...
async function deleteManagedUser(uid){
  ...
  const confirmed =
    await askSystemConfirmation({
      ...
      message:`Excluir o usuário "${user.name || user.email}"? Ele perderá imediatamente o acesso ao sistema.`,
  ...
  try{
    ...
    await Promise.all([
      deleteDoc(doc(FIREBASE_DB,'users',uid)),
      deleteDoc(doc(FIREBASE_DB,'userPresence',uid))
    ]);

    USER_PRESENCE_CACHE.delete(String(uid));
...
/* ============================================================
   CONFIGURAÇÕES
============================================================ */
function firebaseStorageStatus(){
  if(!firebaseConfig.storageBucket){
    return {
      state:'error',
      title:'Bucket não configurado',
      description:'O campo storageBucket não está definido na configuração Firebase.'
    };
  }
  ...
  if(!FIREBASE_STORAGE){
    return {
      state:'warning',
      title:'Aguardando inicialização',
      description:'O SDK do Firebase Storage ainda não foi inicializado.'
    };
  }
  ...
  return {
    ...
    description:`Bucket: ${firebaseConfig.storageBucket}`
  };
}

function storageTestImageBlob(){
  return new Promise((resolve,reject)=>{
    try{
      const canvas = document.createElement('canvas');
      canvas.width = 32;
...
function storageTestErrorMessage(error){
  const code = String(error?.code || '');
  const message = String(error?.message || '');

  if(code.includes('unauthorized')){
    return 'A conexão chegou ao Storage, mas as regras ainda não autorizaram o teste. Publique storage.rules desta versão.';
  }

  if(code.includes('bucket-not-found')){
    return 'O bucket configurado não foi encontrado. Confira o Storage no Firebase Console.';
  }

  if(code.includes('quota-exceeded')){
    return 'O Firebase informou limite de cota do Storage.';
  }
  ...
  if(
    ...
    return 'Não foi possível concluir o teste por problema de conexão.';
  }

  return 'O teste do Firebase Storage não foi concluído. Verifique as regras publicadas e tente novamente.';
}


function storageAutomaticMaintenanceState(){
  return loadLocalKey(
    'storageAutomaticMaintenance',
    {
      status:'never',
      at:'',
      reason:'',
...
function storageMaintenanceSnapshot(){
  const media =
    finalMediaOptimizationInventory();
  ...
  const pending =
    Number(media.legacyPending || 0) +
    Number(media.thumbPending.length || 0);
...
function storageMaintenancePanelHtml(){
  if(!isSystemOwner()) return '';

  const snapshot =
    storageMaintenanceSnapshot();

  const previous =
    storageAutomaticMaintenanceState();

  const statusLabel =
    !navigator.onLine
      ? 'Offline'
      : snapshot.storageStatus.state==='ready'
        ? 'Operacional'
        : 'Atenção';
  ...
  const totalPhotos =
    snapshot.media.storageTotal;

  const readyThumbs =
    snapshot.media.thumbReady;
  ...
  const lastCleanup =
    previous.cleanupAt
      ...
      : 'Automática quando necessária';

  return `
    <div class="card storage-maintenance-card">
      <div class="storage-maintenance-head">
        <div>
          <div class="section-title" style="margin:0 0 4px;">
            Manutenção do Sistema
          </div>
          <h3>Armazenamento e desempenho</h3>
          <p>
            Rotinas de Storage, miniaturas, migração residual e cache
            são verificadas automaticamente em segundo plano.
          </p>
        </div>

        <span class="storage-maintenance-badge ${statusClass}">
          AUTOMÁTICO
        </span>
      </div>

      <div
        class="storage-maintenance-grid"
        id="storage-maintenance-grid">

        <div class="storage-maintenance-metric ${statusClass}">
          <span>Firebase Storage</span>
          <b>${escapeHtml(statusLabel)}</b>
          <small>${escapeHtml(snapshot.storageStatus.title)}</small>
        </div>

        <div class="storage-maintenance-metric">
          <span>Fotos principais</span>
          <b>${totalPhotos}</b>
          <small>extintores + hidrantes</small>
        </div>

        <div class="storage-maintenance-metric ${
          readyThumbs===totalPhotos
            ? 'success'
            : 'warning'
        }">
          <span>Miniaturas</span>
          <b>${readyThumbs}/${totalPhotos}</b>
          <small>cards otimizados</small>
        </div>
        ...
        <div class="storage-maintenance-metric ${
          ...
          <small>tratadas automaticamente</small>
        </div>
      </div>

      <div class="storage-maintenance-status">
        <div>
          <span class="storage-maintenance-dot"></span>
          <div>
            <b>Automação ativa</b>
            <small>
              Verifica ao entrar, após sincronizações e a cada 15 minutos.
            </small>
          </div>
        </div>

        <div class="storage-maintenance-last">
          <span>Última verificação</span>
          <b>${escapeHtml(formatMaintenanceDateTime(previous.at))}</b>
        </div>
      </div>

      <details class="storage-maintenance-details">
        <summary>Detalhes técnicos</summary>

        <div class="storage-maintenance-detail-grid">
          <div>
            <span>Fotos antigas pendentes</span>
            <b>${snapshot.media.legacyPending}</b>
          </div>

          <div>
            <span>Miniaturas pendentes</span>
            <b>${snapshot.media.thumbPending.length}</b>
          </div>

          <div>
            <span>Última limpeza de cache</span>
            <b>${escapeHtml(lastCleanup)}</b>
          </div>

          <div>
            <span>Política de mídia</span>
            <b>WebP + thumbnail 360 px</b>
          </div>
        </div>

        <small class="storage-maintenance-note">
          Ferramentas de migração e limpeza permanecem internas para
          recuperação excepcional, mas não exigem operação manual no uso normal.
        </small>
      </details>
...
function refreshStorageMaintenancePanel(){
  if(
    STATE.view!=='settings' ||
    !isSystemOwner()
  ){
  ...
  bindStorageMaintenancePanel();
}

function bindStorageMaintenancePanel(){
  document
    .getElementById(
    ...
    ?.addEventListener(
      'click',
      event=>{
        runAutomaticStorageMaintenance({
          manual:true,
          reason:'manual',
...
function shouldRunAutomaticCacheCleanup({
  manual=false
}={}){
  ...
  if(!state.cleanupAt) return true;
...
async function runAutomaticStorageMaintenance({
  ...
  if(STORAGE_AUTO_MAINTENANCE_RUNNING){
    if(manual){
      showToast(
        'A manutenção automática já está em andamento.'
      );
    }
  ...
  if(
    ...
    if(manual){
      showToast(
        'A manutenção precisa de conexão com a internet.',
        'err'
      );
    }

    refreshStorageMaintenancePanel();
    return false;
  }
  ...
  if(queue.length){
    ...
    if(manual){
      showToast(
        'Aguardando a sincronização das alterações pendentes.'
      );
    }
  ...
  STORAGE_AUTO_MAINTENANCE_RUNNING =
    true;
  ...
  try{
    await repairStoragePhotoReferenceDrift()
      .catch(error=>{
        console.warn(
          'Reparo automático de referência de mídia adiado.',
          error?.message || error
        );
      });
    ...
    const legacyBatch =
      legacyPhotoMigrationInventory()
        ...
        .slice(
          0,
          STORAGE_AUTO_LEGACY_BATCH_SIZE
        );

    for(const item of legacyBatch){
      try{
        const result =
          await migrateLegacyPhotoItem(
            item
          );
        ...
        console.warn(
          'Migração automática de foto adiada:',
          item?.type,
          item?.record?.id,
          error?.message || error
        );
    ...
    if(!media.legacyPending){
      const thumbBatch =
        media.thumbPending.slice(
          0,
          STORAGE_AUTO_THUMB_BATCH_SIZE
        );
    ...
    if(
      !media.legacyPending &&
      !media.thumbPending.length &&
      shouldRunAutomaticCacheCleanup({
        manual
      })
    ){
      await cleanupObsoleteOperationalCaches({
    ...
    const pendingTotal =
      Number(
        finalState.legacyPending || 0
      ) +
      Number(
        finalState.thumbPending.length || 0
      );
    ...
    saveLocalKey(
      'storageAutomaticMaintenance',
      {
        ...
        failures,
        legacyPending:
          finalState.legacyPending,
        thumbPending:
          finalState.thumbPending.length,
        cleanupAt
      }
    );
    ...
    if(manual){
      if(failures){
        showToast(
          `Manutenção concluída com ${failures} item(ns) para nova tentativa automática.`,
          'err'
        );
      }else if(pendingTotal){
        showToast(
          `Manutenção executada. ${pendingTotal} item(ns) restante(s) serão tratados automaticamente.`,
          'ok'
        );
      }else{
        showToast(
          'Manutenção concluída. Armazenamento e cache estão otimizados.',
          'ok'
        );
      }
    }
    ...
    if(
      ...
      scheduleAutomaticStorageMaintenance({
        delay:
          STORAGE_AUTO_CONTINUATION_DELAY_MS,
        reason:'continuation'
      });
    }
    ...
    console.error(
      'Manutenção automática do Storage falhou:',
      error
    );
    ...
    if(manual){
      showToast(
        'A manutenção não pôde ser concluída agora. O sistema tentará novamente automaticamente.',
        'err'
      );
    }
...
function scheduleAutomaticStorageMaintenance({
  delay=6000,
  reason='scheduled'
}={}){
  if(!isSystemOwner()) return;
  ...
  clearTimeout(
    STORAGE_AUTO_MAINTENANCE_TIMER
  );
    ...
    setTimeout(
      ()=>{
        runAutomaticStorageMaintenance({
          ...
          console.warn(
            'Manutenção agendada não concluída.',
            error?.message || error
          );
        });
      },
...
function startAutomaticStorageMaintenance(){
  if(!isSystemOwner()) return;
  ...
  if(
    STORAGE_AUTO_MAINTENANCE_INTERVAL
  ){
    return;
  }
  ...
  STORAGE_AUTO_MAINTENANCE_INTERVAL =
    setInterval(
      ...
      STORAGE_AUTO_INTERVAL_MS
    );
}

async function testFirebaseStorage(button){
  const statusElement =
    document.getElementById('storage-diagnostic-status');

  if(!FIREBASE_STORAGE || !AUTH_USER){
    if(statusElement){
      statusElement.innerHTML = `
        <b>Storage indisponível</b>
        <small>Entre novamente no sistema e tente o diagnóstico.</small>
      `;
      statusElement.className =
        'storage-diagnostic-status error';
    }

    showToast(
      'Firebase Storage ainda não está disponível.',
      'err'
    );
  ...
  if(statusElement){
    statusElement.innerHTML = `
      <b>Executando diagnóstico</b>
      <small>Upload, leitura e exclusão de um arquivo temporário.</small>
    `;
    statusElement.className =
      'storage-diagnostic-status testing';
  }
  ...
  try{
    ...
    const objectPath =
      `system-tests/${AUTH_USER.uid}/storage-test.${extension}`;
    ...
    if(statusElement){
      statusElement.innerHTML = `
        <b>Storage funcionando corretamente</b>
        <small>Upload, leitura e exclusão concluídos com sucesso.</small>
      `;
      statusElement.className =
        'storage-diagnostic-status success';
    }

    showToast(
      'Firebase Storage conectado e funcionando.',
      'ok'
    );
  }catch(error){
    console.error(
      'Firebase Storage diagnostic error',
      error
    );

    /*
      Se o upload passou mas alguma etapa seguinte falhou,
      tentamos remover o arquivo temporário.
    */
    if(storageReference){
      try{
        const { deleteObject } =
    ...
    const friendlyMessage =
      storageTestErrorMessage(error);

    if(statusElement){
      statusElement.innerHTML = `
        <b>Teste não concluído</b>
        <small>${escapeHtml(friendlyMessage)}</small>
      `;
      statusElement.className =
        'storage-diagnostic-status error';
    }
...
async function continueAfterAuthentication({
  ...
  const cacheReady =
    await loadCachedApplicationState();

  if(cacheReady){
    validateCurrentUserScope();
    applyUserDataScope();
    renderShell();
    startCurrentUserPresence();
    startAutomaticStorageMaintenance();

    openRequestedExtinguisherFromUrl();
    openRequestedHydrantFromUrl();

    /*
      A interface entra primeiro pelo IndexedDB.
      A nuvem é consultada logo depois, sem manter a tela de carregamento.
    */
    setTimeout(()=>{
  ...
  setTimeout(()=>processOfflineQueue(false),450);
  return true;
}
  ...
  content.innerHTML = `
    ...
    ${list.length ? `<div class="hydrant-grid">${list.map(h=>{
      ...
      return `<article class="hydrant-card" data-hydrant-open="${h.id}">
        <div class="hydrant-photo">${equipmentCardPhotoSource(h)?`<img src="${equipmentCardPhotoSource(h)}" alt="" loading="lazy" decoding="async">`:ICONS.hydrant}</div>
...
function openHydrantModal(hydrantId=null){
  ...
  let pendingHydrantStoragePhoto = null;
  let pendingHydrantStoragePreviewUrl = null;
    ...
    <div
      ...
      <div class="modal-body">
        <div class="field">
          ...
          <div
            class="storage-photo-note"
            id="hydrant-photo-note">
  ...
  function clearPendingHydrantStoragePreview(){
    if(pendingHydrantStoragePreviewUrl){
      URL.revokeObjectURL(
        pendingHydrantStoragePreviewUrl
      );

      pendingHydrantStoragePreviewUrl = null;
    }
  }

  function closeHydrantRegistrationModal(){
    clearPendingHydrantStoragePreview();
    closeModal();
  }

  closeButton.onclick =
    closeHydrantRegistrationModal;
    ...
    input?.addEventListener(
      ...
      async event=>{
        ...
        try{
          ...
          const prepared =
            await prepareHydrantPhotoForStorage(
              file
            );

          clearPendingHydrantStoragePreview();

          pendingHydrantStoragePhoto =
            prepared;
          ...
          if(upload){
            upload.innerHTML = `
              <img
                src="${pendingHydrantStoragePreviewUrl}"
                alt="Prévia da nova foto do hidrante">
          ...
          if(note){
            note.textContent =
              `Nova foto pronta para o Storage · ${formatMediaBytes(prepared.optimizedBytes)}`;
          ...
          console.error(
            'Erro ao preparar foto do hidrante:',
            error
          );
          ...
          if(note){
            note.textContent =
              hydrantPhotoStorageErrorMessage(
                error
              );
  ...
  saveButton.onclick = async()=>{
    ...
    try{
      ...
      const commitPendingHydrantStoragePhoto =
        async()=>{
          if(!pendingHydrantStoragePhoto){
            return true;
          }
          ...
          try{
            const uploadedPhoto =
              await uploadHydrantPhotoToStorage(
                record,
                pendingHydrantStoragePhoto
              );
            ...
            if(uploadedPhoto){
              ...
              record.fotoStorageProvider =
                'firebase-storage';

              record.fotoStorageContentType =
                uploadedPhoto.contentType;

              record.fotoStorageBytes =
                uploadedPhoto.bytes;

              record.fotoStorageWidth =
                uploadedPhoto.width;

              record.fotoStorageHeight =
                uploadedPhoto.height;

              record.fotoStorageUpdatedAt =
                new Date().toISOString();

              record.fotoStorageVersion = 1;
            ...
            console.error(
              'Erro no upload da foto do hidrante:',
              photoError
            );
      ...
      if(
        !await commitPendingHydrantStoragePhoto()
      ){
        saveButton.disabled = false;
        saveButton.textContent = originalText;
        return;
...
async function deleteHydrant(hydrantId){
  ...
  if(
    !await askSystemConfirmation({
      ...
      message:`O hidrante #${hydrant.numero} e todo o histórico de inspeções serão removidos.`,
      confirmLabel:'Excluir',
      danger:true
    })
  ){
  ...
  const targetId = String(hydrantId);

  const linked = STATE.hydrantInspections.filter(inspection=>
    String(inspection.hydrantId)===targetId
  );

  try{
    /*
      Primeiro neutraliza qualquer salvamento ainda pendente.
      Isso é importante principalmente quando o hidrante acabou de ser
      cadastrado e o usuário o exclui antes do término da sincronização.
    */
    await discardQueuedUpsert(
      'hydrants',
      targetId
    ...
    for(const inspection of linked){
      await discardQueuedUpsert(
        'hydrantInspections',
        inspection.id
      );
    ...
    STATE.hydrants = STATE.hydrants.filter(item=>
    ...
    /*
      Atualização imediata dos dois caches.
      Assim o cadastro também continua excluído depois de atualizar a página
      enquanto a operação aguarda sincronização.
    */
    await Promise.all([
      cacheOfflineValue(
        scopedOfflineKey('hydrants'),
    ...
    /*
      A exclusão do hidrante já remove as inspeções vinculadas na nuvem,
      mas mantemos também as exclusões individuais para compatibilidade
      com dados antigos e operação offline.
    */
    for(const inspection of linked){
      await persistHydrantInspectionDeletion(
        inspection.id
      );
    }

    await persistHydrantDeletion(targetId,hydrant);

    showToast(
      navigator.onLine
        ? 'Hidrante excluído. Confirmando exclusão na nuvem...'
        : 'Hidrante excluído deste aparelho. A nuvem será atualizada quando houver conexão.'
    );

    renderView();

    /*
      Em conexão ativa não esperamos o temporizador automático:
      tentamos sincronizar imediatamente.
    */
    if(
      navigator.onLine &&
      FIREBASE_READY &&
      ...
      processOfflineQueue(false).catch(error=>{
        console.warn(
          'A exclusão ficou na fila para nova tentativa.',
          error
        );
      });
    }
    ...
    /*
      Recarrega os dados para evitar estado visual inconsistente
      caso a preparação da exclusão tenha falhado.
    */
    try{
      await loadAll();
      applyUserScope();
      ...
      console.warn(
        'Não foi possível recarregar após falha de exclusão.',
        reloadError
      );
    }
  }
    ...
    .forEach(button=>{
      button.addEventListener('click',async()=>{
        ...
        const confirmed =
          await askSystemConfirmation({
            title:'Excluir inspeção',
            message:`Excluir definitivamente a inspeção de ${
              fmtDate(inspection.date)
            }?`,
            confirmLabel:'Excluir',
            danger:true
...
let PWA_REGISTRATION = null;
let PWA_UPDATE_CHECK_PROMISE = null;

function showPwaUpdate(worker){
  PWA_WAITING_WORKER = worker || null;
...
function waitForServiceWorkerSettled(
  registration,
  timeoutMs=2200
){
  return new Promise(resolve=>{
...
async function checkForPwaUpdate({
  ...
  if(!('serviceWorker' in navigator)){
    if(manual){
      showToast(
        'Atualização automática não é suportada neste navegador.',
        'err'
      );
    }
  ...
  if(PWA_UPDATE_CHECK_PROMISE){
    if(manual){
      showToast(
        'Verificação de atualização em andamento.'
      );
    }

    return PWA_UPDATE_CHECK_PROMISE;
  }

  PWA_UPDATE_CHECK_PROMISE = (async()=>{
    try{
      const registration =
        PWA_REGISTRATION ||
      ...
      if(!registration){
        if(manual){
          showToast(
            'Serviço de atualização ainda não está pronto.'
          );
        }
      ...
      PWA_REGISTRATION = registration;

      await registration.update();

      const waiting =
        registration.waiting ||
        await waitForServiceWorkerSettled(
          registration
        );

      if(waiting){
        showPwaUpdate(waiting);
...
async function registerPwa(){
  ...
  try{
    ...
    /*
      Verificação rápida ao abrir, voltar ao app e focar a janela.
      O botão "Atualizar sistema" também força uma consulta manual.
    */
    setTimeout(
      ()=>checkForPwaUpdate(),
      1600
...
document.addEventListener(
  ...
  ()=>{
    document
      .getElementById('pwa-update-now')
      ?.addEventListener(
        'click',
        ()=>{
          if(PWA_WAITING_WORKER){
            PWA_WAITING_WORKER.postMessage({
              type:'SKIP_WAITING'
            });
          }else{
            window.location.reload();
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v4.1.5 · Automação e Painel de Manutenção</div>
          </div>
        </div>
...
function bindMobileProfileMenu(){
  ...
  document.getElementById('mobile-profile-update')?.addEventListener('click', ()=>{
    closeMobileProfileMenu();
    checkForPwaUpdate({
      manual:true
    });
