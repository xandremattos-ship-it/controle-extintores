...
function printSingleLabel(e){
  ...
  holder.innerHTML = `<div class="print-only" style="display:flex;justify-content:center;padding:40px;">
    <div class="qr-label-preview qr-label-simple" style="width:220px;">
      <div id="${qrId}" class="qr-label-code"></div>
      <div class="qr-label-number">#${escapeHtml(e.numero)}</div>
      <div class="qr-label-usina">${escapeHtml((e.usina || "Usina não informada").toUpperCase())}</div>
    </div>
  </div>`;
...
function printAllLabels(requestedUsina=''){
  ...
  holder.innerHTML = `
    <div class="print-only qr-label-sheet">
      ${labels.map(e=>`
        <div class="qr-label-preview qr-label-batch qr-label-simple">
          <div
            id="qr-all-${escapeHtml(String(e.id))}"
            class="qr-label-code">
          </div>
          <div class="qr-label-number small">#${escapeHtml(e.numero)}</div>
          <div class="qr-label-usina small">
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v3.2.13.6 · Etiqueta QR Simplificada</div>
          </div>
        </div>
