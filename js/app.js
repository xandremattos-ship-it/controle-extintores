...
function ncTimelineEventMeta(event){
  ...
  if(type === 'ENCAMINHAMENTO'){
    return {title:'Tratativa registrada e encaminhada', cls:'forwarded'};
  }
  if(type === 'ACOMPANHAMENTO'){
    return {title:'Acompanhamento registrado', cls:'progress'};
  }
  if(type === 'ATUALIZACAO'){
    return {title:'Dados da tratativa atualizados', cls:'updated'};
  }
  if(type === 'STATUS'){
    const statusMeta = ncStatusMeta(status);
    return {
      title:`Status alterado para ${statusMeta.label}`,
      cls:statusMeta.cls
    };
  }
  return {title:'Atualização da tratativa', cls:'updated'};
}
...
function ncTimelineDescription(event,nc){
  ...
  if(type === 'ATUALIZACAO'){
    const fields = Array.isArray(event?.campos) ? event.campos : [];
    return fields.length
      ? `Campos atualizados: ${fields.join(', ')}.`
      : 'As informações da tratativa foram atualizadas.';
  }

  if(type === 'STATUS'){
    const previous = event?.statusAnterior
      ? ncStatusMeta(event.statusAnterior).label
      : '';
    const current = event?.status
      ? ncStatusMeta(event.status).label
      : '';

    if(previous && current){
      return `Status alterado de ${previous} para ${current}.`;
    }
    return current
      ? `A tratativa passou para ${current}.`
      : 'O status da tratativa foi atualizado.';
  }

  return 'Registro de acompanhamento da não conformidade.';
}
...
function normalizedNcTimeline(inspection){
  ...
  const forwardedAt = nc.dataEncaminhamento
    ? `${nc.dataEncaminhamento}T12:05:00`
    : createdAt;
  ...
  if(!history.some(item=>String(item.tipo).toUpperCase()==='DETECCAO')){
    history.push({
      ...
      status:'NC DETECTADA',
      data:createdAt,
      usuario:nc.criadoPor || inspection.responsavel || 'Usuário',
      perfil:''
    });
  }

  if(!history.some(item=>String(item.tipo).toUpperCase()==='ENCAMINHAMENTO')){
    history.push({
      tipo:'ENCAMINHAMENTO',
      status:'ENCAMINHADA',
      data:forwardedAt,
  ...
  const finalStatus = String(nc.status || 'ENCAMINHADA').toUpperCase();
  const hasFinalStatus = history.some(item=>
    String(item.tipo).toUpperCase()==='STATUS' &&
    String(item.status || '').toUpperCase()===finalStatus
  );

  if(finalStatus !== 'ENCAMINHADA' && !hasFinalStatus){
    history.push({
      tipo:'STATUS',
      status:finalStatus,
      data:nc.atualizadoEm || createdAt,
      usuario:nc.atualizadoPor || nc.criadoPor || inspection.responsavel || 'Usuário',
      perfil:''
    });
  }
  ...
  openModal(`
    <div class="modal inspection-view-modal" style="max-width:900px;">
      ...
      <div class="modal-body">
        ...
        ${!isConforme && inspection.naoConformidade ? renderNcTimeline(inspection) : ''}
