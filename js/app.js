...
async function persistExtinguishers(){
  ...
  const pendingItems = STATE.extinguishers.filter(item=>item?._syncStatus === 'pending');

  if(!navigator.onLine || !FIREBASE_READY || !FIREBASE_DB){
    await enqueueOfflineOperation({
      type:'snapshot',
      collection:'extinguishers',
      items:pendingItems.length ? pendingItems : STATE.extinguishers
    });
    return;
  }

  try{
    for(const item of pendingItems.length ? pendingItems : STATE.extinguishers){
      if(!item?.id) continue;
      if(!hasAllUsinasAccess() && item.usina !== currentUserUsina()) continue;

      try{
        const saved = await secureUpsertRecord('extinguishers',item);
        const index = STATE.extinguishers.findIndex(record=>String(record.id)===String(item.id));
        if(index >= 0) STATE.extinguishers[index] = saved;
      }catch(error){
        if(error?.code === 'VERSION_CONFLICT'){
          await saveConflict({
            collectionName:'extinguishers',
            localData:item,
...
async function persistInspections(){
  ...
  const pendingItems = STATE.inspections.filter(item=>item?._syncStatus === 'pending');
  ...
  try{
    for(const item of pendingItems){
      ...
      try{
        const saved = await secureUpsertRecord('inspections',item);
        const index = STATE.inspections.findIndex(record=>String(record.id)===String(item.id));
        if(index >= 0) STATE.inspections[index] = saved;
      }catch(error){
        if(error?.code === 'VERSION_CONFLICT'){
          await saveConflict({
...
function markPendingSync(record){
  ...
  if(record._baseVersion === undefined || record._baseVersion === null){
    record._baseVersion = Number(record.version || 0);
  }

  record.version = Number(record._baseVersion || 0) + 1;
  record._syncStatus = 'pending';
  record._localUpdatedAt = Date.now();
  record._localUserId = AUTH_USER?.uid || '';
  record.updatedBy = AUTH_USER?.uid || '';
  record.updatedByName = CURRENT_USER_PROFILE?.name || AUTH_USER?.email || 'Usuário';
  record.updatedRole = currentRole();
  record.updatedUsina = currentUserUsina();
  record.updatedDevice = typeof deviceFingerprint === 'function' ? deviceFingerprint() : navigator.userAgent;
  record.clientUpdatedAt = new Date().toISOString();
  return record;
}

function pendingSyncBadge(record){
  if(record?._syncStatus === 'conflict' || record?._conflict){
    return `<span class="conflict-badge">Conflito de versão</span>`;
  }
...
async function deleteExtinguisher(id){
  ...
  if(!navigator.onLine || !FIREBASE_READY || !FIREBASE_DB){
    await enqueueOfflineOperation({
      ...
      collection:'extinguishers',
      documentId:id,
      baseVersion:Number(ext.version || 0),
      baseVersion:Number(ext.version || 0)
    });
...
function openInspectionModal(extId, inspectionId=null){
  ...
  const e = STATE.extinguishers.find(x=>x.id===extId);
  if(!e){ showToast("Extintor não encontrado","err"); return; }

  const existing = inspectionId ? STATE.inspections.find(i=>i.id===inspectionId) : null;
  const answers = {};
        ...
        <div class="checklist" id="checklist">
          ${CHECK_ITEMS.map((label,i)=>`
            <div class="check-row">
              ...
              <div class="seg" data-item="${i}">
                ${['C','NC','NA','S.A'].map(v=>`
                  <button data-v="${v}" class="${answers[i]===v?'sel':''}">${v}</button>
                `).join('')}
              </div>
            </div>
          `).join('')}
  ...
  document.getElementById('m-save-insp').addEventListener('click', async ()=>{
    const saveBtn = document.getElementById('m-save-insp');
    const originalText = saveBtn.textContent;
    const responsavel = document.getElementById('f-resp').value.trim();
    ...
    saveBtn.disabled = true;
    saveBtn.textContent = existing ? "Salvando alterações..." : "Salvando inspeção...";

    const previousSnapshot = existing ? JSON.parse(JSON.stringify(existing)) : null;
    let createdInspection = null;

    try{
      const resultado = Object.values(answers).includes('NC') ? 'NÃO CONFORME' : 'CONFORME';
      const updatedData = {
      ...
      if(existing){
        ...
        createdInspection = markPendingSync({
          id:uid('INSP'),
          ...updatedData
        });
        STATE.inspections.push(createdInspection);
      }

      await persistInspections();

      const conflict = existing?._syncStatus === 'conflict';
      if(conflict){
        showToast(
          "A inspeção foi alterada por outro usuário. Revise o conflito antes de continuar.",
          "err"
        );
        openConflictCenter();
        return;
      ...
      showToast(
        existing ? "Inspeção atualizada" : `Inspeção registrada — ${resultado}`,
      ...
      if(createdInspection){
        STATE.inspections = STATE.inspections.filter(
          item=>String(item.id)!==String(createdInspection.id)
        );
      }
      ...
      showToast(
        permissionDenied
          ? "O administrador não possui permissão para alterar esta inspeção nas regras atuais."
          : "Não foi possível salvar as alterações da inspeção.",
        "err"
      );
    }finally{
      saveBtn.disabled = false;
...
async function ensureJsQrAvailable(){
  if(typeof window.jsQR === 'function') return true;
  try{
    return await window.JSQR_READY === true;
  }catch(error){
    console.warn("Falha ao carregar o fallback jsQR:",error);
    return false;
  }
}
...
function openInspectionModal(extId, inspectionId=null){
  ...
  document.getElementById('m-save-insp').addEventListener('click', async ()=>{
    const responsavel = document.getElementById('f-resp').value.trim();
    if(!responsavel){ showToast("Informe o responsável pela inspeção","err"); return; }
    ...
    if(existing){
      ...
      STATE.inspections.push({id:uid('INSP'), ...updatedData});
    }

    await persistInspections();
    showToast(existing ? "Inspeção atualizada" : `Inspeção registrada — ${resultado}`, resultado==='CONFORME'?'ok':'err');
    openExtinguisherDetail(extId);
    renderView();
  });
}
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v3.0.1 Mobile</div>
          </div>
        </div>
