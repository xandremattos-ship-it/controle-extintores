...
async function renderDashboard(){
  ...
  if(ext.length===0){
    content.innerHTML = emptyState(
      ...
      can('manageExtinguishers') ? "Cadastrar extintor" : null
    );
  ...
  try{
    if(!hasAllUsinasAccess()){
      ...
      for(const inspectionDoc of snap.docs){
        ...
        if((data.usina || ext.usina)===currentUserUsina()){
    ...
    if(hasAllUsinasAccess()){
      ...
      saveLocalKey('extinguishers:'+AUTH_USER.uid, STATE.extinguishers);
      saveLocalKey('inspections:'+AUTH_USER.uid, STATE.inspections);
    }
    ...
    console.error(error);
    showToast("Não foi possível excluir o extintor.","err");
  }
}
...
function openExtinguisherDetail(id){
  ...
  document.querySelectorAll('[data-delete-inspection]').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      ...
      await persistInspections();
      showToast("Inspeção excluída","ok");
      openExtinguisherDetail(e.id);
    });
  });
}
