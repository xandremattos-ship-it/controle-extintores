/* ============================================================
   FALLBACK TEMPORÁRIO DO MÓDULO OFFLINE
============================================================ */
if(typeof window.openOfflineDatabase!=='function'){
...
let NOTIFICATION_READ_FLUSH_PROMISE = null;
...
async function flushPendingNotificationReadIds(){
  ...
  if(NOTIFICATION_READ_FLUSH_PROMISE){
    return NOTIFICATION_READ_FLUSH_PROMISE;
  }

  NOTIFICATION_READ_FLUSH_PROMISE = (async()=>{
    ...
    .catch(error=>{
      console.warn(
        'Leitura pendente: será sincronizada novamente.',
        error
      );
      return false;
    })
    .finally(()=>{
      NOTIFICATION_READ_FLUSH_PROMISE = null;
    });
...
async function syncNotificationReadStateFromCloud({
  ...
  NOTIFICATION_READ_SYNC_PROMISE = (async()=>{
    try{
      await Promise.race([
        flushPendingNotificationReadIds(),
...
/* ============================================================
   BOOT
============================================================ */
async function clearApplicationCacheAndReload(){
  try{
    ...
    console.warn('Falha ao limpar o cache do aplicativo.',error);
  }

  const cleanUrl = new URL(window.location.href);
  cleanUrl.searchParams.set('recuperado',Date.now());
  window.location.replace(cleanUrl.toString());
}

function renderBootRecovery(message){
  document.getElementById('app').innerHTML = `
    <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center;">
      <div style="max-width:440px;">
        <h2 style="margin:0 0 10px;">Atualização incompleta</h2>
        <p style="color:var(--muted);line-height:1.55;">
          ${message}
        </p>
        <button class="btn btn-accent" id="boot-clear-cache">
  ...
  document.getElementById('boot-clear-cache')
    ?.addEventListener('click',clearApplicationCacheAndReload);
}
...
async function boot(){
  ...
  const bootWatchdog = setTimeout(()=>{
    ...
    if(
      appRoot &&
      appRoot.textContent.includes('Carregando aplicativo')
    ){
      renderBootRecovery(
        'O navegador ainda está usando arquivos antigos. A limpeza abaixo remove apenas o cache do aplicativo.'
      );
    }
  },12000);

  try{
    if(typeof openOfflineDatabase==='function'){
      await openOfflineDatabase().catch(error=>
        console.error('IndexedDB init error:',error)
      );
    }else{
      console.warn(
        'offline.js não foi carregado. Continuando em modo online.'
      );
    }
  ...
  if(AUTH_USER){
    ...
    renderBootRecovery(
      'Não foi possível concluir a inicialização. Limpe o cache para baixar novamente todos os arquivos.'
    );
  }finally{
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v3.2.13.4 · Modo Offline Restaurado</div>
          </div>
        </div>
