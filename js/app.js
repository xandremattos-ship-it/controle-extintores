...
function extinguisherMatchesSearch(extinguisher,queryValue){
  ...
  const nc = inspection?.naoConformidade || {};
  const number = normalizeExtinguisherSearch(extinguisher.numero);
  ...
  const searchable = normalizeExtinguisherSearch(fields.join(' '));
  const compactSearchable = searchable.replace(/\s+/g,'');
  const tokens = query.split(' ').filter(Boolean);

  if(tokens.length===1 && number===tokens[0]) return true;

  return tokens.every(token=>
    searchable.includes(token) ||
    compactSearchable.includes(token.replace(/\s+/g,''))
  );
}
...
function renderExtinguishers(){
  ...
  const applyExtinguisherSearch = (value,immediate=false)=>{
    extFilters.search = value;
    clearTimeout(EXTINGUISHER_SEARCH_TIMER);
    ...
    EXTINGUISHER_SEARCH_TIMER = setTimeout(execute,120);
  };

  searchInput?.addEventListener('input',event=>{
    applyExtinguisherSearch(event.target.value);
  });

  searchInput?.addEventListener('search',event=>{
    applyExtinguisherSearch(event.target.value,true);
  });
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v3.2.13.6.2 · Busca e NC Corrigidas</div>
          </div>
        </div>
