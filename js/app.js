...
async function startUsersPresenceRealtime(){
  ...
  try{
    ...
    USER_PRESENCE_UNSUBSCRIBE = onSnapshot(
      collection(FIREBASE_DB,'userPresence'),
      snapshot=>{
        USER_PRESENCE_CACHE = new Map(
          snapshot.docs.map(item=>[
      ...
      error=>{
        console.warn(
          'Atualização em tempo real da presença indisponível.',
          error
        );
      }
    );

    USER_PRESENCE_UI_TIMER = setInterval(
      updateUsersPresenceUi,
      USER_PRESENCE_UI_REFRESH_MS
    );
  }catch(error){
    console.warn(
      'Não foi possível iniciar a presença em tempo real.',
      error
    );
  }
}
...
async function loadCollectionData(
  ...
  try{
    ...
    const snap = serverOnly
      ? await getDocsFromServer(sourceQuery)
      : await getDocs(sourceQuery);
...
async function deleteHydrant(hydrantId){
  ...
  try{
    ...
    */
    ...
    await persistHydrantDeletion(targetId);
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v4.0.9 · Sincronização e Desempenho</div>
          </div>
        </div>
