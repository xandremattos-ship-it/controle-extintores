...
function extinguisherSearchValues(extinguisher){
  const inspections = STATE.inspections.filter(inspection=>
    String(inspection.extinguisherId)===String(extinguisher.id)
  );

  const inspectionValues = inspections.flatMap(inspection=>{
    const nc = inspection?.naoConformidade || {};
  ...
  return [
    extinguisher.numero,
    `#${extinguisher.numero ?? ''}`,
    String(extinguisher.numero ?? '').replace(/^0+/,''),
    extinguisher.localizacao,
    extinguisher.usina,
    extinguisher.agente,
...
function extinguisherMatchesSearch(extinguisher,queryValue){
  ...
  const searchable = normalizeExtinguisherSearch(
    extinguisherSearchValues(extinguisher).join(' ')
  );
  ...
  return queryTokens.every(token=>{
    ...
    return (
      ...
      compactSearchable.includes(compactToken)
    );
  });
}
...
function applyExtinguisherDomSearch(queryValue){
  ...
  if(empty){
    empty.hidden = !(query.trim() && resultElements.length && visibleCount===0);
  }
}
...
function renderExtinguishers(){
  ...
  content.innerHTML = `
    ...
    <div class="empty" id="ext-search-empty" hidden>
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v3.2.13.6.2.1 · Busca de Extintores Corrigida</div>
          </div>
        </div>
