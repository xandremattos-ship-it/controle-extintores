...
function renderSettings(){
  ...
  content.innerHTML = `
    <div class="grid grid-2">
      ...
      <div class="card storage-diagnostic-card">
        <div class="section-title" style="margin-top:0;">
          Firebase Storage
        </div>

        <div class="storage-diagnostic-intro">
          <div>
            <span>Parte 1 · Base de mídia</span>
            <h3>Conexão e segurança</h3>
          </div>

          <span class="storage-diagnostic-badge">
            STORAGE ATIVO
          </span>
        </div>

        <p class="storage-diagnostic-copy">
          Fotos novas de extintores e hidrantes já utilizam o Storage.
          A Parte 4 abaixo permite migrar com segurança as fotos antigas
          que ainda estão incorporadas ao Firestore.
        </p>

        <div
          class="storage-diagnostic-status ${firebaseStorageStatus().state}"
          id="storage-diagnostic-status">
          <b>${escapeHtml(firebaseStorageStatus().title)}</b>
          <small>${escapeHtml(firebaseStorageStatus().description)}</small>
        </div>

        <div class="storage-diagnostic-paths">
          <div>
            <span>Extintores</span>
            <code>extintores/{id}/foto.webp</code>
          </div>

          <div>
            <span>Hidrantes</span>
            <code>hidrantes/{id}/foto.webp</code>
          </div>
        </div>
        ...
        <small class="storage-diagnostic-help">
          O diagnóstico cria uma imagem temporária, confirma a leitura
          e apaga o arquivo logo em seguida.
        </small>
      </div>

      <div class="card storage-migration-card">
        <div class="section-title" style="margin-top:0;">
          Migração das fotos antigas
        </div>

        <div class="storage-migration-head">
          <div>
            <span>Parte 4 · Otimização da base existente</span>
            <h3>Firestore → Firebase Storage</h3>
          </div>

          <span class="storage-migration-safe">
            MIGRAÇÃO CONTROLADA
          </span>
        </div>

        <p class="storage-diagnostic-copy">
          O sistema identifica somente fotos antigas incorporadas
          como Data URL. A migração ocorre em lotes de até
          ${LEGACY_PHOTO_MIGRATION_BATCH_SIZE} arquivos para evitar
          sobrecarga e facilitar a conferência.
        </p>

        <div id="storage-migration-stats">
          ${storagePhotoMigrationStatsHtml()}
        </div>

        <div class="storage-migration-safety">
          <b>Proteção contra perda de foto</b>
          <small>
            A imagem original só é substituída no Firestore depois
            que o upload e a URL do Storage forem confirmados.
            Se uma foto falhar, ela permanece intacta para uma nova tentativa.
          </small>
        </div>

        <div
          class="storage-migration-progress"
          id="storage-migration-progress">

          <div class="storage-migration-progress-head">
            <span data-storage-migration-label></span>
            <b data-storage-migration-count></b>
          </div>

          <div class="storage-migration-track">
            <i data-storage-migration-bar></i>
          </div>
        </div>
        ...
        <button
          type="button"
          class="btn btn-accent"
          id="storage-migrate-batch"
          style="width:100%;justify-content:center;"
          ${legacyPhotoMigrationInventory().pending.length ? '' : 'disabled'}>
          ${
            legacyPhotoMigrationInventory().pending.length
              ? `Migrar próximo lote · até ${Math.min(
                  LEGACY_PHOTO_MIGRATION_BATCH_SIZE,
                  legacyPhotoMigrationInventory().pending.length
                )} foto(s)`
        ...
        <small
          class="storage-diagnostic-help"
          id="storage-migration-summary">
          ${
            legacyPhotoMigrationInventory().pending.length
              ? `${legacyPhotoMigrationInventory().pending.length} foto(s) antiga(s) aguardando migração.`
              : 'Nenhuma foto antiga incorporada permanece pendente.'
          }
        </small>
      </div>

      <div class="card storage-final-card">
        <div class="section-title" style="margin-top:0;">
          Limpeza e otimização final
        </div>

        <div class="storage-final-head">
          <div>
            <span>Parte 5 · Desempenho de mídia</span>
            <h3>Miniaturas + cache enxuto</h3>
          </div>

          <span class="storage-final-badge">
            OTIMIZAÇÃO FINAL
          </span>
        </div>

        <p class="storage-diagnostic-copy">
          As telas de listagem passam a usar miniaturas WebP leves de até
          ${STORAGE_THUMB_MAX_DIMENSION}px. A foto principal continua preservada
          em qualidade maior para prontuários e relatórios.
        </p>

        <div id="storage-final-stats">
          ${finalMediaOptimizationStatsHtml()}
        </div>

        <div
          class="storage-migration-progress"
          id="storage-final-progress">

          <div class="storage-migration-progress-head">
            <span data-storage-final-label></span>
            <b data-storage-final-count></b>
          </div>

          <div class="storage-migration-track">
            <i data-storage-final-bar></i>
          </div>
        </div>

        <div class="storage-final-actions">
          <button
            type="button"
            class="btn btn-accent"
            id="storage-optimize-thumbnails"
            ${finalMediaOptimizationInventory().thumbPending.length ? '' : 'disabled'}>
            ${
              finalMediaOptimizationInventory().thumbPending.length
                ? `Otimizar próximo lote · até ${Math.min(
                    STORAGE_THUMB_BATCH_SIZE,
                    finalMediaOptimizationInventory().thumbPending.length
                  )} miniatura(s)`
          ...
          <button
            type="button"
            class="btn"
            id="storage-final-cleanup"
            ${
              (
                finalMediaOptimizationInventory().legacyPending ||
                finalMediaOptimizationInventory().thumbPending.length
              )
                ? 'disabled'
                : ''
        ...
        <small
          class="storage-diagnostic-help"
          id="storage-final-summary">
          ${
            finalMediaOptimizationInventory().legacyPending
              ? 'Conclua primeiro a migração das fotos antigas da Parte 4.'
              : finalMediaOptimizationInventory().thumbPending.length
                ? `${finalMediaOptimizationInventory().thumbPending.length} equipamento(s) ainda precisam de miniatura leve.`
                : 'Fotos e miniaturas estão otimizadas. A limpeza final já pode ser executada.'
          }
        </small>
      </div>
  ...
  document
    .getElementById('storage-run-test')
  ...
  document
    .getElementById('storage-migrate-batch')
    ?.addEventListener('click',event=>{
      migrateLegacyPhotoBatch(
        event.currentTarget
      );
    });

  document
    .getElementById('storage-optimize-thumbnails')
    ?.addEventListener('click',event=>{
      optimizeStorageThumbnailBatch(
        event.currentTarget
      );
    });

  document
    .getElementById('storage-final-cleanup')
    ?.addEventListener('click',event=>{
      runStorageFinalCleanup(
        event.currentTarget
      );
    });
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
          ...
          <div>
            <div style="font-family:var(--font-display);font-size:18px;font-weight:700;">Gestão de Extintores</div>
            <div style="font-size:11px;color:var(--muted);margin-top:4px;">PWA Enterprise v4.1.4 · Limpeza e Otimização Final</div>
          </div>
        </div>
