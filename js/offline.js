...
async function applyQueuedDelete(operation){
  ...
  if(operation.collection==='hydrants'){
    const targetId =
      String(operation.documentId);

    const storagePaths =
      Array.isArray(operation.storagePaths)
        ? operation.storagePaths.filter(Boolean)
        : [];
    ...
    if(
      ...
      try{
        ...
        for(const storagePath of storagePaths){
          try{
            ...
            if(
              ...
              console.warn(
                'A foto do hidrante não pôde ser removida do Storage.',
        ...
        console.warn(
          'Módulo Storage indisponível durante a exclusão do hidrante.',
          storageModuleError?.message ||
          storageModuleError
        );
      }
    }
...
async function processOfflineQueue(manual=false){
  ...
  if(!eligible.length){
    ...
    */
    ...
    if(
      typeof scheduleAutomaticStorageMaintenance===
        'function'
    ){
