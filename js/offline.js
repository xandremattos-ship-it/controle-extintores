...
async function cacheCurrentOfflineSession(){
  ...
  await cacheOfflineValue(scopedOfflineKey('hydrants'),STATE.hydrants);
  await cacheOfflineValue(scopedOfflineKey('hydrantInspections'),STATE.hydrantInspections);
}
...
async function applyQueuedDelete(operation){
  ...
  if(operation.collection === 'hydrants'){
    await deleteDoc(doc(FIREBASE_DB,'hydrants',String(operation.documentId)));
  ...
  const eligible = queue
    ...
    .sort((a,b)=>{
      ...
      const priority = operation=>{
        ...
        if(operation.type==='upsert' && operation.collection==='hydrants') return 0;
        if(operation.type==='upsert' && operation.collection==='inspections') return 1;
        if(operation.type==='upsert' && operation.collection==='hydrantInspections') return 1;
        return 2;
      };
