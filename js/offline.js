...
async function resolveConflictOverwrite(conflictId){
  ...
  try{
    ...
    const ref = doc(
      FIREBASE_DB,
      conflict.collection,
      String(conflict.documentId)
    );

    // Lê novamente a versão atual antes da substituição.
    const beforeSnapshot = await getDoc(ref);
    const beforeData = beforeSnapshot.exists() ? beforeSnapshot.data() : null;
    const currentServerVersion = Number(beforeData?.version || 0);
    const localClean = cleanRecordForCloud(conflict.localData);
    const nextVersion = Math.max(
      currentServerVersion,
      Number(localClean.version || 0),
      Number(conflict.serverVersion || 0)
    ) + 1;

    const cloudPayload = {
      ...localClean,
      id:String(conflict.documentId),
      version:nextVersion,
      updatedBy:AUTH_USER?.uid || '',
      updatedByName:CURRENT_USER_PROFILE?.name || AUTH_USER?.email || 'Usuário',
      updatedRole:typeof currentRole === 'function' ? currentRole() : '',
    ...
    // Grava explicitamente no Firestore.
    await setDoc(ref,cloudPayload,{merge:true});

    // Confirma que a gravação realmente chegou à nuvem antes de apagar o conflito.
    const verificationSnapshot = await getDoc(ref);
    if(!verificationSnapshot.exists()){
      throw new Error('CLOUD_WRITE_NOT_CONFIRMED');
    }

    const verifiedData = verificationSnapshot.data();
    const verifiedVersion = Number(verifiedData.version || 0);

    if(verifiedVersion !== nextVersion){
      throw new Error(
        `CLOUD_VERSION_NOT_CONFIRMED: expected ${nextVersion}, received ${verifiedVersion}`
      );
    }
    ...
    await writeAuditEntry({
      ...
      action:`Substituiu ${conflict.collection === 'extinguishers' ? 'extintor' : 'inspeção'} pela versão local`,
      before:beforeData,
      after:{...cloudPayload,updatedAt:null},
      forced:true
    });

    const target = conflict.collection === 'extinguishers'
      ? STATE.extinguishers
      : STATE.inspections;
    ...
    delete confirmedRecord._syncStatus;
    delete confirmedRecord._baseVersion;
    delete confirmedRecord._conflict;
    delete confirmedRecord._localUpdatedAt;
    delete confirmedRecord._localUserId;

    const index = target.findIndex(
      item=>String(item.id)===String(conflict.documentId)
    );

    if(index >= 0) target[index] = confirmedRecord;
    else target.push(confirmedRecord);

    const key = scopedOfflineKey(conflict.collection);
    await cacheOfflineValue(key,target);
    saveLocalKey(key,target);

    // Somente remove o conflito depois da confirmação no Firestore.
    await idbDelete('conflicts',conflictId);

    showToast('Versão local confirmada e gravada na nuvem.');
    await renderConflictCenter();
    await updateConnectionUi();

    // Recarrega a coleção da nuvem para validar o estado final.
    if(typeof refreshDashboardCollections === 'function'){
      await refreshDashboardCollections();
    }
    if(typeof cacheCurrentOfflineSession === 'function'){
    ...
    showToast(
      'A versão local não foi confirmada na nuvem. O conflito foi mantido para nova tentativa.',
      'err'
    );
  }
}
  ...
  for(const operation of eligible){
    ...
    try{
      ...
      if(operation.type === 'snapshot'){
        const snapshotResult = await applyQueuedSnapshot(operation);
        const conflictTotal = Number(snapshotResult?.conflicts || 0);
        SYNC_CONFLICTS += conflictTotal;
        operationHasConflict = conflictTotal > 0;

        if(!operationHasConflict){
          clearSyncedRecordFlags(operation.collection);
        }
      }else if(operation.type === 'delete'){
        const deleteResult = await applyQueuedDelete(operation);
        operationHasConflict = deleteResult?.conflict === true;
        if(operationHasConflict) SYNC_CONFLICTS++;
      ...
      // Um conflito exige decisão humana e não deve ser tratado como sucesso.
      // A fila é removida porque a cópia local já foi preservada em "conflicts".
